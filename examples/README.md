...
The current regression record is 42/42 passing at release
`tt-transformers==2.0.0.dev0`: modules 30/30, runtime 1/1,
...
`--machine-class` names the capability class the host belongs to, as listed
under `machines` in the matrix. The runner does not take a hostname: it checks
the class you declare against the device count, board types and cluster type
your `tt-smi -s` inventory actually reports. A class that selects devices by
BDF also requires `selected_bdfs` in that inventory.

Nodes that load a checkpoint need a Hugging Face cache. Pass `--hf-home`, or
export `HF_HOME`; the runner refuses rather than guessing a location.
...
MACHINE_CLASS="<machine class from tests/hardware/hardware-matrix.json>"
...
host's machine class and physical-inventory file in the variables
defined above:
