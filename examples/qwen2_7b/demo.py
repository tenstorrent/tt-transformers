...
def _skip_below_min_tp_devices(n_devices: int) -> None:
    ...
    if n_devices < _MIN_TP_DEVICES:
        raise UnsupportedConfiguration(f"Qwen2-7B requires >={_MIN_TP_DEVICES}-device tensor parallelism: the unsharded 7B overflows a single device's L1 (matmul circular-buffer clash). TTTv1/PERF.md publish this checkpoint N300-only. Have {n_devices} device(s) — use MESH_DEVICE=N300.")
...
def load_reference_data(hf_model_id: str):
    ...
    ref_path = Path("qualification/assets/reference_outputs/qwen2_7b") / f"{name}.refpt"
...
def _dp_lane_tp_or_skip(mesh_device: ttnn.MeshDevice, data_parallel: int) -> int:
    ...
    if tensor_parallel != _MIN_TP_DEVICES:
        raise UnsupportedConfiguration(f'DP-{data_parallel} on {n} devices creates TP{tensor_parallel} lanes; Qwen2-7B requires TP{_MIN_TP_DEVICES} lanes')
