...
Usage:
    # Token accuracy test
    MESH_DEVICE=N300 HF_MODEL=Qwen/Qwen2-7B-Instruct pytest tests/hardware/models/qwen2_7b/test_demo.py -k "token-accuracy" -v

    # Batch-1 latency test
    MESH_DEVICE=N300 HF_MODEL=Qwen/Qwen2-7B-Instruct pytest tests/hardware/models/qwen2_7b/test_demo.py -k "batch-1" -v

    # On-device sampling perf sweep
    SAMPLING_MODE=on_device_topk MESH_DEVICE=N300 HF_MODEL=Qwen/Qwen2-7B-Instruct \
      pytest tests/hardware/models/qwen2_7b/test_demo.py -k "batch-32-ci" -v
...
``tests/assets/reference_outputs/qwen2_7b/Qwen2-7B-Instruct.refpt``, generated fresh for
...
from tt_transformers.models.qwen2_7b.hf_generator import Qwen2Executor, Qwen2ExecutorConfig, _load_model
...
#                    regression -> the host gate is this implementation's own value, and the
#                    host-vs-host comparison is informational rather than a gate.
...
def _skip_below_min_tp_devices(n_devices: int) -> None:
    ...
    if n_devices < _MIN_TP_DEVICES:
        raise UnsupportedConfiguration(
            f"Qwen2-7B requires >={_MIN_TP_DEVICES}-device tensor parallelism: the unsharded 7B overflows a single device's L1 (matmul circular-buffer clash). TTTv1/PERF.md publish this checkpoint N300-only. Have {n_devices} device(s) — use MESH_DEVICE=N300."
        )
...
def ttnn_mesh_device_param_from_env() -> dict:
    ...
    # src/tt_transformers/models/qwen2_7b/demo.py wiring: FABRIC_1D on any >1-device mesh.
    if shape != (1, 1):
...
def load_reference_data(hf_model_id: str):
    ...
    ref_path = Path("tests/assets/reference_outputs/qwen2_7b") / f"{name}.refpt"
...
def _dp_lane_tp_or_skip(mesh_device: ttnn.MeshDevice, data_parallel: int) -> int:
    ...
    if tensor_parallel != _MIN_TP_DEVICES:
        raise UnsupportedConfiguration(
            f"DP-{data_parallel} on {n} devices creates TP{tensor_parallel} lanes; Qwen2-7B requires TP{_MIN_TP_DEVICES} lanes"
        )
...
def _validate_dp_lane(model: Qwen2_7B, lane: Qwen2Executor, tensor_parallel: int, max_seq_len: int) -> None:
    ...
    if attention.n_heads % tensor_parallel or attention.n_kv_heads % tensor_parallel:
        raise ValueError(
            f"DP lane TP{tensor_parallel} does not divide Qwen2 heads ({attention.n_heads}/{attention.n_kv_heads})"
        )
    ...
    try:
        for submesh in submeshes:
            try:
                ...
                raise UnsupportedConfiguration(f"Could not build Qwen2-7B TP2 lane (weights / memory / mesh): {error}")
            model = llm.model
            model.demo_tokenizer = llm.tokenizer
        ...
        if not (len(result.generated_token_ids) == data_parallel):
            raise AssertionError("condition failed at line 798")
...
def run_qwen2_7b(test_config, mesh_device, optimizations):
