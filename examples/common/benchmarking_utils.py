...
class BenchmarkData:
    ...
    def add_measurement(
        ...
        if IS_CI_ENV:  # no-op if not running in CI environment
            ...
            """
            if None in [profiler, iteration, step_name, name, value]:
                raise ValueError("Missing required measurement fields")
            if not profiler.contains_step(step_name, iteration):
                raise ValueError(f"Completed step '{step_name}' for iteration {iteration} not found in profiler")
    ...
    def save_partial_run_json(
        ...
        if IS_CI_ENV:  # no-op if not running in CI environment
            if None in [profiler, run_type, ml_model_name]:
                raise ValueError("Missing required run fields")
            if not profiler.contains_step("run"):
                raise ValueError("Run step not found in profiler")
