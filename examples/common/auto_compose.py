...
def to_torch_auto_compose(tensor: ttnn.Tensor, device: ttnn.MeshDevice | None = None) -> torch.Tensor:
    """
...
def get_device_from_tensor(tensor: ttnn.Tensor) -> ttnn.MeshDevice | None:
...
def _infer_mesh_composer_from_topology(
    tensor: ttnn.Tensor, *, device: ttnn.MeshDevice | None = None
) -> ttnn.CppMeshToTensor | None:
    """
    ...
    if mesh_device is None:
        raise RuntimeError(
            "Tensor is on host and no mesh_device was provided. Pass device=... to to_torch_auto_compose."
        )

    # Must match length (should be guaranteed by C++ TT_FATAL in ttnn/core/distributed/distributed_tensor.cpp)
    if len(dist_shape) != len(placements):
        raise ValueError("mesh distribution shape and placements must have equal rank")
...
def _compose_nd_sharded(
    ...
    for i, p in enumerate(placements):
        if isinstance(p, ttnn.PlacementShard):
            ...
            if not isinstance(p, ttnn.PlacementReplicate):
                raise TypeError(f"unsupported mesh placement: {type(p).__name__}")
