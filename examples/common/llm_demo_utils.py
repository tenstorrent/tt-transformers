...
from examples.common.benchmarking_utils import BenchmarkData, BenchmarkProfiler
from examples.common.model_targets import (
...
def create_benchmark_data(profiler: BenchmarkProfiler, measurements: dict, N_warmup_iter: dict, targets: dict):
    ...
    if missing:
        raise ValueError(f"missing required measurements: {sorted(missing)}")
...
def check_tokens_match(generated_text: dict, expected_greedy_output_path: str):
    with open(expected_greedy_output_path) as f:
