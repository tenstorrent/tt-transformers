...
def _skip_unless_heads_divide_mesh(mesh_device: ttnn.MeshDevice, hf_model_id: str) -> None:
    ...
    if n_dev in (1, 2, 8):
        return
    raise UnsupportedConfiguration(f'Incompatible mesh for {hf_model_id}: Llama-3.2-3B supports 1, 2, or 8 devices, got {n_dev}')
...
def load_reference_data(hf_model_id: str):
    ...
    ref_path = Path("qualification/assets/reference_outputs/llama32_3b") / f"{name}.refpt"
...
def _dp_tp_devices_or_skip(mesh_device: ttnn.MeshDevice, data_parallel: int) -> int:
    ...
    if tp_devices not in (1, 2, 8):
        raise UnsupportedConfiguration(f'DP-{data_parallel} on {n} devices creates TP{tp_devices} lanes, but Llama-3.2-3B supports TP1, TP2, or TP8')
    return tp_devices
