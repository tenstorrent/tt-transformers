...
Usage::
    ...
    MESH_DEVICE=N300 HF_MODEL=meta-llama/Llama-3.2-3B-Instruct \\
      pytest tests/hardware/models/llama32_3b/test_demo.py -k "token-accuracy" -v

    # Batch-1 latency test
    MESH_DEVICE=N300 HF_MODEL=meta-llama/Llama-3.2-3B-Instruct \\
      pytest tests/hardware/models/llama32_3b/test_demo.py -k "batch-1" -v

    # Batch-32 throughput test
    MESH_DEVICE=N300 HF_MODEL=meta-llama/Llama-3.2-3B-Instruct \\
      pytest tests/hardware/models/llama32_3b/test_demo.py -k "batch-32" -v
...
reference at ``tests/assets/reference_outputs/llama32_3b/<basename(HF_MODEL)>.refpt``
...
from tt_transformers.models.llama32_3b.hf_generator import Llama32_3BExecutor, Llama32_3BExecutorConfig, _load_model
...
#
# Parity rule: each ``tok_s_u`` target is the BETTER of freshly-measured same-box TTTv1 vs TTTv2 for
...
def ttnn_mesh_device_param_from_env() -> dict:
    ...
    if shape is None:
        raise UnsupportedConfiguration(f"Unsupported MESH_DEVICE={env!r} for Llama-3.2-3B; use N150, N300 or T3K.")
    param = {
        "mesh_shape": shape,
...
def _skip_unless_heads_divide_mesh(mesh_device: ttnn.MeshDevice, hf_model_id: str) -> None:
    ...
    raise UnsupportedConfiguration(
        f"Incompatible mesh for {hf_model_id}: Llama-3.2-3B supports 1, 2, or 8 devices, got {n_dev}"
    )
...
def load_reference_data(hf_model_id: str):
    ...
    ref_path = Path("tests/assets/reference_outputs/llama32_3b") / f"{name}.refpt"
...
def create_model(
    ...
    *,
    ...
    try:
        ...
        raise UnsupportedConfiguration(f"Could not build Llama-3.2-3B model (weights / memory / mesh): {e}")
...
def _dp_tp_devices_or_skip(mesh_device: ttnn.MeshDevice, data_parallel: int) -> int:
    ...
    if tp_devices not in (1, 2, 8):
        raise UnsupportedConfiguration(
            f"DP-{data_parallel} on {n} devices creates TP{tp_devices} lanes, but Llama-3.2-3B supports TP1, TP2, or TP8"
        )
    return tp_devices
...
def _run_dp_smoke(
    ...
    try:
        ...
        if not (len(result.generated_token_ids) == data_parallel):
            raise AssertionError("condition failed at line 685")
...
def run_llama32_3b(test_config, mesh_device, optimizations):
