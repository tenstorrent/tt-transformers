# Llama 3.3 70B on WH Galaxy with TTTv2

The 2D tensor-parallel reconstruction of `meta-llama/Llama-3.3-70B-Instruct` for one Wormhole Galaxy
`(8, 4)` mesh — 32 boards, 8 rows × 4 columns.

The package is `tt_transformers.models.llama33_70b_galaxy`; the shared Galaxy runtime it builds on is
...
**No `demo.py`.** The demo that existed upstream was a Milestone B *direct* demo: it drove the model
...
that supersedes it is exercised by `tests/models/llama33_70b_galaxy/test_executor_wh_galaxy.py` and
...
| Surface | Location |
| --- | --- |
| Model, weights, HF adaptor, executor | `src/tt_transformers/models/llama33_70b_galaxy/` |
...
| Host tests | `tests/models/llama33_70b_galaxy/*_host.py` |
| Device tests (need a Galaxy mesh; no matrix node selects them yet) | `tests/models/llama33_70b_galaxy/*_wh_galaxy.py` |
...
- Milestone C did not pass on performance. The paired decode gates failed by a factor rather than a
  margin; the one paired gate that passed was Llama TTFT, at −76.57 %. That investigation was
  deliberately left behind rather than ported mid-flight.

## Running the host tests

```sh
pytest tests/models/llama33_70b_galaxy -m host
```
