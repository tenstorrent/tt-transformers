...
EXPECTED_METRICS_BATCH1: dict = {
    ...
    "on_device_topk": {
        ...
        # ~1.4% under; a diffuse shared-engine per-step delta, NOT lowered to a slow number).
        ...
        # not lowered). b1 TTFT noisy (~118-127ms both stacks); 145 ceiling covers ON+OFF.
    },
}
...
# (a diffuse shared-engine per-step delta). The gate stays at the prior-healthy TTTv1 best-of;
...
# (batched-ON ~40/49 << the sequential-OFF ~103/113); the tolerance-free parity RED is tracked
# separately rather than folded into a lowered gate.
EXPECTED_METRICS_BATCH32_CI: dict = {
    ...
    "on_device_topk": {
        ...
        # under (diffuse shared-engine per-step delta; NOT lowered to the TTTv2 number).
        # ttft is a CEILING TTTv2 clears (minimal_matmul-ON batched ~40/49 << the sequential-OFF ~103/113);
        # the tolerance-free TTFT parity RED is documented alongside the shared-gap ticket.
...
def run_qwen3_32b(test_config, mesh_device, optimizations):
...
def run_qwen3_32b_p150x4_seeded_cross_cardinality(mesh_device):
