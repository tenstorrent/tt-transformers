...
The immediate prerequisite to implementation is agreement on the two-mode interface and confirmation of the CPU build environment/cache arrangement. This document recommends editable TTNN for development spanning both repositories and fixed wheels for reproducible CI. Neither the proposed source-linked build/rebuild loop nor a current-main TTNN wheel has been validated yet.
