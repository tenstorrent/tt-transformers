...
def validate_matrix(matrix: dict[str, Any], root: Path = ROOT) -> dict[str, int]:
    ...
    for node in nodes:
        ...
        if "required_evidence_fields" in node:
            raise MatrixError(f"{node_id} redeclares required_evidence_fields; the matrix-level list is authoritative")
...
def validate_attestation(
    ...
    *,
    ...
    if machine_class not in node["machine_pool"]:
        raise MatrixError(f"machine class {machine_class!r} is not in the pool for node {node['id']}")
    if machine_class not in matrix["machines"]:
        raise MatrixError(f"unknown machine class {machine_class!r}")
...
def validate_physical_inventory(
    ...
    # Site identity must not reach behaviour: the class declares what it requires,
    # and the caller supplies the values for its own chassis.
    capabilities = machine.get("capabilities", [])
    requires_unset = machine.get("tt_visible_devices") == "must_be_unset"
    if node["mesh_device"] == "P150":
        ...
        if requires_unset and inventory["tt_visible_devices"] not in {None, ""}:
            raise MatrixError(
                f"{machine_name} P150 requires TT_VISIBLE_DEVICES unset; MESH_DEVICE=P150 is the sole topology selector"
            )
    if node["mesh_device"] == "P150x4":
        ...
        if "revalidated_bdf_ring" in capabilities:
            if len(set(inventory.get("selected_bdfs", []))) != 4:
                raise MatrixError(f"{machine_name} P150x4 requires a revalidated four-BDF Ring")
...
def expand(value: str, *, checkout: Path, run_root: Path, node: dict[str, Any], hf_home: str | None) -> str:
    if "{hf_home}" in value:
        if not hf_home:
            raise MatrixError("this node needs a Hugging Face cache: pass --hf-home or set HF_HOME")
        value = value.replace("{hf_home}", hf_home)
    return (
...
def environment_for(
    ...
    *,
    ...
    # The class states its device-selection policy; the concrete BDFs are a
    # property of the caller's chassis and arrive with the physical inventory.
    policy = machine.get("tt_visible_devices")
    if policy == "must_be_unset":
        values["TT_VISIBLE_DEVICES"] = None
    elif policy == "from_selected_bdfs":
        selected = inventory.get("selected_bdfs") or []
        if not selected:
            raise MatrixError(f"{machine_name} selects devices by BDF; physical inventory lacks selected_bdfs")
        values["TT_VISIBLE_DEVICES"] = ",".join(selected)
    cache = node["cache_requirement"]
    if cache["kind"] == "writable_node_local":
        values["TT_CACHE_PATH"] = cache["path"]
    return {
        key: expand(value, checkout=checkout, run_root=run_root, node=node, hf_home=hf_home)
...
def preview(
    ...
    validate_physical_inventory(matrix, node, machine_name, inventory)
    run_root = args.output_dir.resolve()
    environment = environment_for(
        node,
        matrix["machines"][machine_name],
        machine_name,
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--machine-class")
    result.add_argument("--hf-home", help="Hugging Face cache for nodes that need one; defaults to $HF_HOME")
...
def main() -> int:
    ...
    required = {
        ...
        "--branch": args.branch,
        "--machine-class": args.machine_class,
