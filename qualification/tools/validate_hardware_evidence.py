...
def resolve_machine(matrix: dict[str, Any], node: dict[str, Any], caller: Any, path: Path) -> str:
    if not isinstance(caller, str) or not caller:
        raise EvidenceError(f"{path}: caller_machine_identity must be non-empty")
...
def template_matches(template: Any, actual: Any, *, node: dict[str, Any]) -> bool:
    ...
    for literal, field_name, format_spec, conversion in string.Formatter().parse(template):
        ...
        if field_name is None:
            continue
        if format_spec or conversion or field_name not in {"checkout", "run_root", "node_id", "mesh_device"}:
            return False
        if field_name in {"checkout", "run_root"}:
            pieces.append(r".+")
...
def validate_record(
    *,
    ...
    machine_name = resolve_machine(matrix, node, evidence.get("caller_machine_identity"), evidence_path)
    validate_actual_fqdn(matrix, machine_name, evidence.get("actual_fqdn"), path=evidence_path)
