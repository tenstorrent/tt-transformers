...
TARGETS = {
    "base-py310": (Path("/usr/local/share/uv/cpython-3.10.19-linux-x86_64-gnu/bin/python3.10"), "constraints/locks/base-py310.txt"),
    "host-py310": (Path("/usr/local/share/uv/cpython-3.10.19-linux-x86_64-gnu/bin/python3.10"), "constraints/locks/host-py310.txt"),
    "qualification-py310": (Path("/usr/local/share/uv/cpython-3.10.19-linux-x86_64-gnu/bin/python3.10"), "constraints/locks/qualification-py310.txt"),
    "build-dev-py310": (Path("/usr/local/share/uv/cpython-3.10.19-linux-x86_64-gnu/bin/python3.10"), "constraints/locks/build-dev-py310.txt"),
    "base-py312": (Path("/tmp/gwang/uv-python/cpython-3.12.13-linux-x86_64-gnu/bin/python3.12"), "constraints/locks/base-py312.txt"),
    "host-py312": (Path("/tmp/gwang/uv-python/cpython-3.12.13-linux-x86_64-gnu/bin/python3.12"), "constraints/locks/host-py312.txt"),
    "qualification-py312": (Path("/tmp/gwang/uv-python/cpython-3.12.13-linux-x86_64-gnu/bin/python3.12"), "constraints/locks/qualification-py312.txt"),
    "build-dev-py312": (Path("/tmp/gwang/uv-python/cpython-3.12.13-linux-x86_64-gnu/bin/python3.12"), "constraints/locks/build-dev-py312.txt"),
}
...
def main() -> int:
    ...
    validations = []
    for target, (interpreter, relative_lock) in TARGETS.items():
        ...
        torch_absence_probe = (
            run(
                [str(python), "-c", "import importlib.util; raise SystemExit(importlib.util.find_spec('torch') is not None)"],
                env=env,
            )
            if installed.returncode == 0 and not requires_torch
            else None
        )
        tt_transformers_probe = run(
            [str(python), "-c", "import importlib.util; raise SystemExit(importlib.util.find_spec('tt_transformers') is not None)"],
            env=env,
        ) if installed.returncode == 0 else None
        passed = bool(
            installed.returncode == 0
            and checked is not None
            and checked.returncode == 0
            and (
                (requires_torch and torch_probe is not None and torch_probe.returncode == 0 and torch_probe.stdout.strip() == "2.11.0+cpu")
