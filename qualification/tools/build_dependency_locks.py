...
EVIDENCE_PATH = ROOT / "qualification/reports/dependency-lock-evidence.json"
TARGETS = {
...
def parse_report(path: Path, *, expected_python: str, require_torch: bool) -> tuple[dict, list[dict]]:
    ...
    if require_torch and (
        len(torch) != 1 or torch[0]["version"] != "2.11.0+cpu" or torch[0]["index"] != "pytorch_cpu"
    ):
...
def main() -> int:
    ...
    if EVIDENCE_PATH.is_file():
        ...
        validations.update(
            {
                lock["target"]: lock["strict_install_validation"]
                for lock in previous.get("locks", [])
                if lock.get("strict_install_validation") is not None
            }
        )
    ...
    if not EVIDENCE_PATH.is_file() or EVIDENCE_PATH.read_text() != encoded:
        mismatches.append(str(EVIDENCE_PATH.relative_to(ROOT)))
    if mismatches:
