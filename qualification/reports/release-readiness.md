...
Destination candidate: `tt-transformers==0.1.0.dev0` at pushed commit `2883a949860d749adc2ed1af5525b27a9a547505`

## Verdict

**Not release ready.** The standalone package builds, installs non-editably, imports, and passes its complete host suite on both supported Pythons against `ttnn==0.77.0`. At the pushed final candidate SHA, all 34 accessible hardware nodes pass, including the strict T3K W6 runtime trace/order node. Eight single-P150 nodes require a different unavailable machine, and a separate noncanonical four-cell T3K diagnostic passes every TTFT ceiling but misses every throughput floor. All model manifests remain experimental, while external consumer cutover, final release tagging, hardware CI, explicit-cache qualification, and non-green quality cleanup also remain open.
...
| partial | 18 | A useful subset, including exact-SHA hardware coverage, passes, but the completion criterion is not fully met |
| blocked | 16 | Deferred geometry, hardware CI, external-consumer, or prerequisite work prevents closure |
...
```text
Validated 79 release-readiness criteria: 45 pass, 18 partial, 16 blocked; 12/12 models remain experimental and not release-qualified.
```
...
- The audited wheel rebuilt from final candidate `2883a949860d749adc2ed1af5525b27a9a547505` is 597,778 bytes with SHA-256 `8c55fac0a764fb9ae4d6ca514062ef2cb6cfe3097306a2f877bcad41269c50c2`; the normalized 498,499-byte sdist is `f161e13dedc5ce076d9553b677f0a1a4785996f932316f2325de9217376da644`. Two `SOURCE_DATE_EPOCH` builds are byte-identical, and the 132-file source digest is `7b03baf498e2a2252759d89813fcb898dd88daf573fb46f0e77e5c2cc6abad97`.
...
- The exact full host command passes from the non-editable wheel, without `PYTHONPATH`, on both CPython 3.10 and 3.12: 2,165 passed, 28 intentional skips, 6,791 deselected, 5 warnings, 81 subtests, zero failures/errors on each interpreter.
...
- All 1,462 test functions have an explicit lane: 1,208 host and 254 device; 393 are concrete model surfaces. The current taxonomy audit has zero errors.
...
- Final-SHA device execution covers 34 of 42 matrix nodes, and all 34 pass: modules 23/23, runtime 1/1, smoke 3/3, and end-to-end 7/7. The canonical index SHA-256 is `ec9a540a8f31762078b592909e02cdb03e99384f9ddf8f955966fa41e42af07b`.
- A separate noncanonical four-cell T3K accuracy/TTFT diagnostic passes latency 4/4 but reports throughput performance-floor failures 4/4. It is retained for performance triage and is not counted among the 34 canonical hardware nodes.
...
- The deterministic TTNN verdict remains partial: host compatibility and every executed canonical hardware node pass, but the deferred single-P150 geometry and observed throughput-floor failures prevent a complete hardware/performance verdict.

## Release blockers

| Blocker | Exact current evidence | Owner | Next gate |
|---|---|---|---|
| Hardware matrix is incomplete | Of 42 final-SHA nodes, all 34 accessible nodes pass, including strict W6; priorities 24–30 and 38 are eight single-P150 nodes restricted to the unavailable `bh-lb-11` development loudbox. All twelve manifests remain experimental. | Hardware qualification | Execute the eight deferred P150 nodes on the required hardware. |
| T3K throughput floors fail | The noncanonical accuracy/TTFT diagnostic passes latency in all four cells (`86.7`–`87.2` ms against a `105.0` ms ceiling), but all four miss their adjusted per-user throughput floors; measured aggregate throughput is `245.4`–`389.2` tok/s. | Runtime performance owner | Profile the host and on-device-topk paths, resolve or formally rebaseline the throughput targets, then rerun the four cells. |
...
| 3 — boundary closure | partial | Import, optional-dependency, cache, device ownership, and TTTv1 bridge policies pass statically and on host; all 34 executed final-SHA device nodes pass, including strict W6, while eight P150 nodes remain deferred and performance floors remain open. | Resolve the throughput floor and complete the deferred geometry. |
| 4 — TTNN 0.77 qualification | partial | Both wheel matrices, all symbols, and full host semantics pass on both Pythons. Final-SHA modules are 23/23, runtime 1/1, smoke 3/3, and end-to-end 7/7; eight P150 nodes are deferred and four diagnostic throughput floors fail. | Triage the binding leak and throughput gap, execute deferred P150 nodes, and extend the verdict. |
| 5 — test/CI pyramid | partial | Taxonomy, two-Python host evidence, and all 34 accessible manual WH/BH nodes pass; throughput diagnostics fail 4/4 and consolidated CI plus scheduled/release hardware workflows remain unobserved. | Resolve throughput, require the host workflow, and implement reservation-aware hardware CI. |
...
| 9 — post-extraction cleanup | partial/deferred | A precise failing static-quality baseline exists, but cleanup is correctly deferred until hardware parity: Ruff 2,158, Ruff format 167, mypy 489/99, and bounded Black timeout. | After parity, reduce reviewed categories incrementally and make them zero-regression gates. |

## Twelve-model readiness

Every package is concrete, imports from the installed wheel, participates in the clean host suite, and has a runnable CLI. Five have partial destination-SHA model evidence, but all twelve manifests remain `experimental` with empty evidence arrays, so none is release-qualified.
...
The three Blackhole capability contracts remain pre-acceptance requirements rather than results by themselves. Separate final-SHA physical P150_X4 evidence now supports partial Llama 3.3 70B and Qwen3 32B rows; it does not promote their experimental manifests or establish Llama 3.1 8B support.
...
| Reviewed TTNN 0.77 verdict | partial | Deterministic host evidence and 34/34 executed final-SHA device passes exist; eight deferred P150 nodes and four throughput-floor failures keep the verdict incomplete. |
...
| Qualified examples pass declared host/hardware gates | blocked | Five models have partial attributable evidence, but all twelve manifests remain experimental and none passes its complete promoted contract. |
...
| Current direct/vLLM behavior preserved | partial | Host characterization, strict W6, and seven end-to-end device nodes pass; vLLM server behavior, throughput floors, and unexecuted model geometries remain open. |
...
2. Resolve the four T3K throughput-floor failures and run priorities 24–30 and 38 on the required `bh-lb-11` single-P150 development loudbox.
...
Until those gates close, the accurate release statement is: **host-compatible with TTNN 0.77.0, with all 34 accessible final-SHA hardware nodes passing and five partially evidenced models; eight single-P150 nodes and four throughput floors remain open, all manifests and hardware support remain experimental and unqualified, and downstream cutover has not occurred.**
