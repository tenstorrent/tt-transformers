...
"""Host-resolved 2D attention recipes for a Galaxy `(8, 4)` mesh.

Both Galaxy architectures: the mesh shape and every weight split are identical,
and the architecture gate in `resolve_attention2d_config` is the shared Galaxy
allowlist from `modules/galaxy_mesh.py`. What differs per architecture is the
intra-chip geometry, which arrives already resolved in the config.
"""
...
from tt_transformers.modules.galaxy_mesh import galaxy_architecture_family, is_galaxy_architecture_name
...
from tt_transformers.modules.rmsnorm.rmsnorm_2d import RMSNorm2D, RMSNorm2DConfig, RMSNorm2DGeometry

#: Local copies of the shared Galaxy constants, kept rather than imported.
#: Identical on both Galaxy architectures, so nothing here is architecture
#: dependent -- but the duplication is real and is a separate cleanup; see the
#: same pair in `mlp_2d.py` and `rmsnorm_2d.py`. Only the *architecture* gate
#: below is shared, because that is the one that was wrong.
...
def resolve_attention2d_config(config: Attention2DConfig) -> Attention2DConfig:
    ...
    # `e48c329` widened six 2D modules' mesh gates onto `modules/galaxy_mesh.py`
    # and missed this one -- the plan's own inventory lists `Attention2D`'s
    # architecture-string check separately from the six, and it stayed
    # Wormhole-only. It is the same gate, on the same allowlist, for the same
    # reason: Blackhole Galaxy is a 32-device `(8, 4)` mesh declaring the same
    # `device_topology { dims: [8, 4] }`, so refusing it by name refused a mesh
    # this module can describe.
    #
    # A substring test over an allowlist rather than an equality test, because
    # `architecture` holds an `Arch` enum at one call site and the bare string
    # `"wormhole"` at another -- the same reason `is_galaxy_architecture_name`
    # exists and is shared with `Sampling2DConfig.architecture`.
    for label, name in (("requested", requested), ("mesh", detected)):
        if name and not is_galaxy_architecture_name(name):
            raise ValueError(
                f"Attention2D requires a Galaxy architecture; {label}={name!r} is not one "
                f"(requested={requested!r}, mesh={detected!r})"
            )
    family = galaxy_architecture_family(requested)
    if family is None:
        raise ValueError("Attention2D could not resolve an architecture from the config or the mesh")
    ...
    return replace(
        ...
        wqkv_bias=bias,
        # The architecture the gate above accepted, normalized to its family.
        # Writing the literal `"wormhole"` here made the resolved config
        # disagree with the mesh on Blackhole, and this field is what
        # downstream architecture checks read -- including the
        # `RMSNorm2DConfig.architecture` of the QK-norm sub-modules this config
        # builds. The *family* rather than `requested` itself because the field
        # has one canonical value while the gate accepts every spelling:
        # `Arch.WORMHOLE_B0` must still store `"wormhole"`.
        architecture=family,
        dim=dim,
        qkv_size=qkv_size,
...
class Attention2D(LightweightModule):
    ...
    def _validate_decode_page_table(self, table: Any, binding: KVCacheBinding) -> None:
        ...
        repeated once per core.

        This check discriminates on row count alone, and the modulo is
        deliberate: an L1-sharded decode table legitimately repeats the
        device-local batch once per core. **It therefore does not reject a
        prefill-shaped table.** A replicated prefill table's device-local view
        is a multiple of ``users_per_column``, so it passes here and fails at
        the first op instead. Shape cannot separate the two layouts; the
        discriminator that would is ``memory_config()``, which this validator
        does not consult. The reverse direction — a decode table handed to
        prefill — does fail closed.
        """
    ...
    def release(self) -> None:
        ...
        this module allocates *per request*. The weights outlive that, and before
        this method existed nothing released them: a caller still holding the model - a
    ...
    def decode_forward(self, x: Any, rot_mats: Any, metadata: DecodeMetadata) -> Any:
        ...
        try:
            ...
            if low.gather_users is not None:
                # The user gather's destination may be a buffer the collective's
                # resource owner lends out rather than one the op allocated, and
                # a lent buffer must not be freed here: it is a fabric write
                # target whose address a captured program keeps. Ask, the way the
                # QKV reduction above already does.
                gathered_users = low.gather_users(
                    attention, mode="decode", recipe=None, config=cfg, prefix_bounds=(lower, upper)
                )
                attention = self._transition(
                    attention,
                    gathered_users,
                    borrowed=low.is_borrowed_output(gathered_users),
                )
            concat_kwargs = {
                "num_heads": cfg.n_heads // GALAXY_MESH_SHAPE[0],
