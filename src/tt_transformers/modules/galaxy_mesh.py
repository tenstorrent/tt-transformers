...
def is_galaxy_architecture_name(architecture: Any) -> bool:
    ...
    return galaxy_architecture_family(architecture) is not None


def galaxy_architecture_family(architecture: Any) -> str | None:
    """Return the Galaxy architecture *family* a name belongs to, or `None`.

    The family is the canonical short token -- `"wormhole"` or `"blackhole"` --
    not the spelling the caller happened to use. `Arch.WORMHOLE_B0`,
    `"wormhole_b0"` and `"wormhole"` all resolve to `"wormhole"`.

    This exists because a config field that *stores* an architecture needs one
    canonical value, while the gate that *checks* one has to accept every
    spelling. `Attention2DConfig.architecture` is the case: it is written once
    during resolution and then read by downstream comparisons, including the
    `RMSNorm2DConfig.architecture` of the QK-norm sub-modules it builds. Writing
    the full enum name there would leave those comparing `"wormhole_b0"`
    against `"wormhole"`.
    """

    text = str(architecture).lower()
    return next((name for name in GALAXY_ARCHITECTURE_NAMES if name in text), None)
