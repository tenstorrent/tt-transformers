...
class RMSNorm2D(LightweightModule):
    ...
    def _decode_head_local(self, x: ttnn.Tensor, residual: ttnn.Tensor | None) -> ttnn.Tensor:
        ...
        * **interleaved input.** ``ttnn.rms_norm`` resolves
          ...
          This is the other side of an earlier fix, which made interleaved DRAM
          the *default* for this geometry. That is right for prefill - whose
          mode plan is one sub-device over the full grid - and unplaceable for
          decode. Nothing had run the decode side until the Qwen3-32B bring-up.
...
def _resolve_2d_config(config: RMSNorm2DConfig) -> RMSNorm2DConfig:
    ...
    if mesh_device is None:
        mesh_device = compatibility_default_device(ttnn, owner="modules.rmsnorm._resolve_2d_config")
    ...
    if geometry is RMSNorm2DGeometry.HEAD_LOCAL:
        ...
        # that disjointness, which is how a second attempt at this failed on
