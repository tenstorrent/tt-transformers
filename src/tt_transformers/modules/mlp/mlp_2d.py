...
class MLP2DConfig:
    ...
    #: Whether decode stages 1-2 may use the fused galaxy CCLs.
    #:
    #: `True` -- the default, and the hardware-qualified Wormhole path -- takes
    #: `llama_rs_matmul` for the W1/W3 projections plus their axis-1 reduction,
    #: and `llama_reduce_scatter` for W3's. `False` takes two `ttnn.linear`
    #: calls and two `reduce_scatter_minimal_async` calls instead, which is the
    #: spelling `prefill_forward` already uses for the same math.
    #:
    #: **It exists because those two ops silently no-op on Blackhole.** Blackhole
    #: Galaxy requires a 2D-torus fabric, a 2D fabric uses a different packet
    #: header, and both ops are 1D-multicast writers -- so they move no data and
    #: raise nothing, and the collective *appears* to run. The other five decode
    #: stages are already non-fused, so this is the whole of the difference.
    #:
    #: A capability injected through the config, not an architecture check:
    #: `modules/` never imports `models/`, where `GalaxyCapabilities` lives, and
    #: the caller sets this from `has_fused_ccl`. Keeping it a separate axis from
    #: the prefetcher matters for work this milestone is not doing -- the
    #: reference needs prefetcher-on with fused-collectives-off at the same time,
    #: which one combined flag could not express.
    decode_fused_ccl: bool = True
...
class MLP2D(LightweightModule):
    ...
    def release(self) -> None:
        ...
        against 208 896 B for the attention `wqkv` ring copy - and before this
        method existed nothing released them at all. `MLP2D` had no `release` and no
    ...
    def _all_reduce_tg(
        ...
        # Stage an unsharded input into the collective's own placement.
        #
        # `all_reduce_async` admits **only** a `WIDTH_SHARDED` input, and says so
        # of its input, its caller-owned persistent buffer *and* its output
        # config (`all_reduce_async_device_operation.cpp:46-57`). On Blackhole it
        # additionally refuses a DRAM input outright, with its own architecture
        # guard at `:28` -- *"does not support blackhole dram as it does not use
        # an accessor to get the noc address as needed by the fabric api"*. So
        # there is no interleaved spelling of this op on any architecture and no
        # DRAM one here at all.
        #
        # Until now this method relocated its input **only as a side effect of a
        # dtype conversion**: the `to_memory_config(..., memory_config)` above
        # sits inside `if input_tensor.dtype != ccl_dtype`. Production survives
        # that because `decode_w2_output_memcfg` defaults to
        # `L1_WIDTH_SHARDED_MEMORY_CONFIG`, so `w2_out` already carries the right
        # placement and the branch never has to matter. A caller whose `w2`
        # emits `ccl_dtype` directly into DRAM skips the branch and hands the
        # collective a DRAM tensor, which is what aborted the Blackhole
        # DRAM-interleaved decode arm on 2026-09-17.
        #
        # `models/galaxy/collectives.py::_all_reduce` already stages
        # unconditionally for exactly this reason -- two implementations of one
        # contract, and only one of them was robust to its input. This makes them
        # agree.
        #
        # No qualified value moves: production reaches here already sharded, so
        # the guard is false and nothing is inserted. It also declines to act
        # when `memory_config` is itself interleaved, because staging *into* an
        # interleaved config would not satisfy the op either -- better to let it
        # refuse with its own message than to add a copy that changes nothing.
        if memory_config is not None and memory_config.is_sharded() and not input_tensor.memory_config().is_sharded():
            staged = ttnn.interleaved_to_sharded(input_tensor, memory_config, output_dtype=ccl_dtype)
            if input_tensor is not original_input:
                ttnn.deallocate(input_tensor)
            input_tensor = staged
        ...
        *,
        ...
        if not persistent:
            return ttnn.reduce_scatter(
                ...
                num_workers_per_link=ccl_context.reduce_scatter_workers_per_link(resources.num_links),
            )
        kwargs = dict(
    ...
    def _unfused_decode_w1_w3(self, x: ttnn.Tensor) -> tuple[ttnn.Tensor, ttnn.Tensor]:
        """Project and reduce W1/W3 for decode without a fused galaxy CCL.

        The same math `_double_matmul_reduce_scatter_axis1` plus
        `_llama_reduce_scatter_axis1` do, spelled the way `prefill_forward`
        already spells it: two `ttnn.linear` calls, then two reductions through
        `_reduce_scatter_axis1`, which routes to
        `reduce_scatter_minimal_async` -- a standard/stable collective with a
        2D-fabric kernel, not one of the 1D-multicast writers.

        Selected by `decode_fused_ccl=False`; read that field's docstring for
        why it exists. Keeping this a separate method rather than inlining the
        branch keeps `decode_forward`'s stage numbering readable and means the
        fused path's code is untouched.

        The two reductions carry the sequence keys ``"w1"`` and ``"w3"``, as
        prefill's do, because they are two distinct collectives on
        same-shaped tensors -- without distinct keys they would alias one
        resource and the second would reuse the first's persistent buffer
        mid-flight. The fused path needs no such key: it is one op.
        """
        ...
        projections = [
            ttnn.linear(
                ...
                **_prefetch_kwargs(cfg.decode_prefetch_context),
            )
            for weight in (self.w1, self.w3)
        ]
        reduced = []
        for projection, key in zip(projections, ("w1", "w3")):
            reduced.append(self._reduce_scatter_axis1(projection, cfg.ff1_out_reduce_scatter_memcfg, "decode", key))
            ttnn.deallocate(projection)
        return reduced[0], reduced[1]

    def decode_forward(self, x: ttnn.Tensor | LazyWeight) -> ttnn.Tensor:
        """
        Galaxy decode forward.

        Unified Path: linear → linear → reduce_scatter(×2) → mul+silu → all_gather → linear → all_reduce

        Stages 1-2 have two spellings, selected by `decode_fused_ccl`. Every
        other stage is already non-fused, so that flag is the whole of the
        difference between the Wormhole and Blackhole decode graphs here.
        """
        ...
        if cfg.decode_fused_ccl:
            ...
            # --- STAGE 1-2: unfused, because the fused pair no-ops on a 2D fabric ---
            w1_out, w3_out = self._unfused_decode_w1_w3(x)
            if owns_input:
                ttnn.deallocate(x)
...
def _resolve_mlp2d_config(config: MLP2DConfig) -> MLP2DConfig:
    ...
    if not config.decode_fused_ccl and config.collective_resource_selector is None:
        # The unfused decode path issues TWO axis-1 reduce-scatters, for W1 and
        # W3, and both outputs are live at once -- stage 3 multiplies them. With
        # no selector, `_select_collective_resources` ignores the sequence key
        # and returns the same `decode_reduce_scatter_resources` for both, so
        # they would share one persistent output buffer and the multiply would
        # read W3 twice. Nothing raises; the answer is just wrong.
        #
        # The fused path does not need a key because it is one op, which is why
        # this is checked here rather than unconditionally.
        raise ValueError(
            "decode_fused_ccl=False issues two distinct axis-1 reduce-scatters whose outputs are "
            "live simultaneously, so it requires a collective_resource_selector that honours the "
            "'w1'/'w3' sequence keys. Without one both reductions alias a single persistent buffer "
            "and the gated multiply silently reads the same tensor twice."
        )
    ...
    if mesh_device is None:
        mesh_device = compatibility_default_device(ttnn, owner="modules.mlp._resolve_mlp2d_config")
    if config.mesh_device is None:
        to_set["mesh_device"] = mesh_device

    cluster_shape = require_galaxy_mesh("MLP2D", mesh_device)
