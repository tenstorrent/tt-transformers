...
def _resolve_rope_config(config: Rope1DConfig) -> Rope1DConfig:
    ...
    if device is None:
        device = compatibility_default_device(ttnn, owner="modules.rope._resolve_rope_config")
...
def prepare_rot_idxs(
    ...
    Useful for pre-allocating host tensors for trace mode before
    `RotarySetup1D.decode_forward`.
