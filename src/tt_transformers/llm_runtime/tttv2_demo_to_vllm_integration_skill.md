...
Read `examples/llama3_8b/README.md` and its
`{hf_adaptor,model,executor,generator}.py` as the package/lifecycle reference.
Read `tests/llm_runtime/test_executor_integration.py` for exact
facade contracts and `src/tt_transformers/llm_runtime/vllm_adapter.py` for external
normalization and late KV binding. Read R1-R6 and the test contract in
`tttv2_sibling_model_vllm_registration_plan.md`, plus
`<vllm-checkout>/tttv2_vllm_integration_policy_design.md`.
...
- Keep special treatment inside the target model package. If a reproduced
  three-tier failure appears to require `src/tt_transformers/llm_runtime/`,
  `src/tt_transformers/modules/`, or `VLLMAdapter`, write the reduction and proposed
...
- Use `<tt-metal-checkout>/python_env`. Use
...
```bash
MODEL_DIR=src/tt_transformers/models/<model>
rg -n "from_pretrained|DEFAULT_HF_REVISION|mesh|cluster_shape|tt_data_parallel|max_batch|max_context|block_size|trace|prefill|sampling|fabric|cache" "$MODEL_DIR" tests/hardware/models
rg -n "<HF model ID>|<generator class>" <vllm-checkout>/plugins/vllm-tt-plugin/src/vllm_tt_plugin/platform.py
```
...
```bash
PYTHONPATH=<tt-metal-checkout> \
  <tt-metal-checkout>/python_env/bin/pytest -q \
  tests/llm_runtime/test_executor_integration.py
```
...
```text
<vllm-checkout>/plugins/vllm-tt-plugin/src/vllm_tt_plugin/platform.py
```
