...
from tt_transformers.llm_runtime.prefill.sampling_helpers import _TILE_SIZE, SamplingPath
from tt_transformers.llm_runtime.prefill.signatures import PreparedPrefill
from tt_transformers.modules.sampling.params import PreparedSamplingParams, prepare_sampling_params
    ...
    def finish_regular_prefill(
        ...
        *,
        ...
        if request.kind == "batched":
            ...
            # recorded the decode traces, leaving buffers live across replays. With the device-side
            # row pick the logits already hold the last token in row 0.
            # Ported from tenstorrent/tt-metal#55343.
            logits = self.config.model.post_process_prefill_output(
                hidden,
                relative_last[0],
