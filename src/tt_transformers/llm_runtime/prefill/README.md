...
`src/tt_transformers/llm_runtime/prefill` implements the common host-side prefill
...
`tests/llm_runtime/test_prefill_runtime.py`. It covers planning,
padding, page-table semantics, signatures, eager sequences, trace refresh,
sampling, result collection, and cleanup failures.

`tests/llm_runtime/test_execution.py` covers the boundary between
...
tested in `tests/llm_runtime/test_warmup.py`.
