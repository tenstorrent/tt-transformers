...
`src/tt_transformers/llm_runtime` is a reusable execution toolkit for TTTv2 language
...
[`src/tt_transformers/models/executor.py`](../models/executor.py). A demonstrated
...
composition in `src/tt_transformers/models` when the lifecycle is demonstrated to be
