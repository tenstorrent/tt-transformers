...
def _load_model(
    ...
    *,
    ...
    if num_devices != 2:
        raise ValueError(f"Phi-4 supports logical TP2 lanes only, got {num_devices} devices")
    load_kwargs = dict(revision=hf_revision, local_files_only=offline_mode())
    ...
    try:
        ...
        del hf
        llm = Phi4ForCausalLM(model=model, tokenizer=tokenizer, runtime_config=runtime_config, instruct=instruct)
...
def from_pretrained(
    ...
    *,
    ...
    paged_attention_config: Phi4PagedAttentionConfig | None = None,
    cache_dir: Path | str | None = None,
    executor_config: Phi4ExecutorConfig | None = None,
) -> Phi4ForCausalLM:
    ...
    the existing ``Phi4ExecutorConfig`` for explicit trace/sampling policy.
    """
    if executor_config is not None and not isinstance(executor_config, Phi4ExecutorConfig):
        raise TypeError("executor_config must be a Phi4ExecutorConfig")
    ...
    try:
        ...
        llm.executor = build_phi4_executor(llm, executor_config)
        try:
