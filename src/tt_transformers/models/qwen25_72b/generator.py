...
"""vLLM construction and compatibility delegation for Qwen2.5-72B."""
...
from tt_transformers.models.qwen25_72b.executor import Qwen25_72BExecutorConfig, build_qwen25_72b_executor
from tt_transformers.models.qwen25_72b.hf_adaptor import DEFAULT_HF_REVISION, from_pretrained
from tt_transformers.models.qwen25_72b.model import Qwen25_72BPagedAttentionConfig
...
class Qwen25_72BGenerator:
    ...
    def initialize_vllm_model(
        ...
        return build_qwen25_72b_generator(
...
def build_qwen25_72b_generator(config: Qwen25_72BGeneratorConfig) -> Qwen25_72BGenerator:
    ...
    try:
        for submesh in submeshes:
            ...
            lanes.append(build_qwen25_72b_executor(llm, executor_config))
    ...
    return Qwen25_72BGenerator(target, adapter)
...
def _model_kv_metadata(model) -> tuple[tuple[Any, ...], int, int, int]:
    ...
    if not layers:
        raise ValueError("Qwen2.5-72B model must contain at least one attention layer")
...
def _attention_config(layer):
    ...
    if hasattr(layer, "config"):
        return layer.config
    raise AttributeError("Qwen2.5-72B layer must expose attention/self_attn.config or config")
