...
"""
Host-side HF weight layout helpers for Qwen2.5-Coder-32B-Instruct (TTTv2 port).
...
without importing the legacy tt-metal model tree. Logic is shape-agnostic and mirrors
the Qwen2.5-7B port; the Qwen2 family checkpoint exposes QKV biases but no
``q_norm`` / ``k_norm`` (those are Qwen3).
"""
