...
DEFAULT_HF_REVISION = "c170c708c41dac9275d15a8fff4eca08d52bab71"
...
def from_pretrained(
    ...
    *,
    ...
    paged_attention_config: Mistral7BPagedAttentionConfig | None = None,
    cache_dir: Path | str | None = None,
    executor_config: Mistral7BExecutorConfig | None = None,
) -> Mistral7BForCausalLM:
    ...
    the existing ``Mistral7BExecutorConfig`` for explicit trace/sampling policy.
    """
    if executor_config is not None and not isinstance(executor_config, Mistral7BExecutorConfig):
        raise TypeError("executor_config must be a Mistral7BExecutorConfig")
    ...
    if paged_attention_config is None:
        ...
        paged_attention_config = Mistral7BPagedAttentionConfig(block_size=block_size, max_num_blocks=max_num_blocks)
    if executor_config is not None:
    ...
    try:
        ...
        llm.executor = build_mistral_7b_executor(llm, executor_config)
        try:
