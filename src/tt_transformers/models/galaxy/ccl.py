...
"""Shared, host-testable ownership for Galaxy CCL resources.

Architecture-neutral: the resource keys, the canonical axes and the ownership
protocol are properties of the `(8, 4)` Galaxy mesh, which both Galaxy
architectures share. The one architecture-dependent fact here is the allowlist
in `_validate_galaxy`, and it is delegated to the topology descriptor.
"""
...
from tt_transformers.models.galaxy.topology import reduce_scatter_workers_per_link, supported_galaxy_architectures
...
def _validate_galaxy(mesh_device: Any, mesh_shape: tuple[int, int], architecture: Any) -> None:
    """Fail closed unless this is a Galaxy mesh we have intra-chip geometry for.

    The architecture check generalizes; nothing else does. It generalizes to
    *"a topology descriptor exists for this architecture"* rather than to a
    wider allowlist, which is the same contract `recipes.validate_galaxy_mesh`
    keeps and for the same reason: an architecture can only pass here once
    someone has written its geometry down. Widened against the error it
    actually raised on silicon -- ``Galaxy CCL requires Wormhole B0, got
    Arch.BLACKHOLE`` -- which is how the six module gates were widened, rather
    than ahead of a measurement.

    The mesh shape stays a hard equality. Blackhole Galaxy declares the same
    ``device_topology { dims: [8, 4] }``, so `(8, 4)` is architecture-invariant
    on Galaxy and only the arch line bites.
    """

    if architecture not in supported_galaxy_architectures():
        supported = ", ".join(str(arch) for arch in supported_galaxy_architectures())
        raise ValueError(f"Galaxy CCL has no topology for {architecture}; supported: {supported}")
    if mesh_shape != (8, 4):
...
class GalaxyModeResources:
    ...
    #: The loaded worker sub-device's core set. `ttnn.SubDevice` exposes no
    #: accessor, so it travels with the plan; `GalaxyModePlan` carries the same
    #: field for the same reason. Only `reduce_scatter` needs it, and only
    #: because it sizes its worker selection against the envelope it was given.
...
class GalaxyCCLConfig:
    ...
    def __post_init__(self) -> None:
        _validate_galaxy(self.mesh_device, self.mesh_shape, self.architecture)
...
class GalaxyCCLContext:
    ...
    def reduce_scatter_workers_per_link(self, num_links: int) -> int | None:
        """Size `ttnn.reduce_scatter`'s worker selection for this mode's partition.

        Pass the result to the op as `num_workers_per_link`. It is the only
        collective that needs this: its program factory offsets the worker walk
        by the loaded sub-device's own origin, which the walk already starts
        from, so a partition that does not begin at the grid origin loses its far
        column and the op aborts on placement before moving an element. Every
        `all_gather` factory passes a zero offset instead.

        `None` means the partition starts at the grid origin and nothing shifts,
        so the op's own data-sized choice is placeable and is left alone. That is
        every Wormhole partition this op is legal under, which is what makes the
        call unconditional at the call sites.

        Returns `None` when no core set travelled with the plan. A module cannot
        recover it from a `ttnn.SubDeviceId`, so a caller that assembles mode
        resources by hand and runs this op on a shifted partition gets the op's
        default and the abort that goes with it -- which is loud, and named in
        `reduce_scatter_workers_per_link`'s own message.
        """

        cores = self._mode_resources.worker_cores
        ranges = getattr(cores, "ranges", None)
        if not callable(ranges):
            return None
        rects = tuple(
            (int(core_range.start.x), int(core_range.start.y), int(core_range.end.x), int(core_range.end.y))
            for core_range in ranges()
        )
        return reduce_scatter_workers_per_link(rects, num_links)
