...
module tests established. Precision, provider conversion, and
...
#: The architecture the no-argument helpers in this module describe, which is
#: the *default* and not the only one. It reads through `_DEFAULT_TOPOLOGY` so
#: the two can never disagree.
#:
#: The name was `GALAXY_ARCHITECTURE` and asserted that Galaxy *is* Wormhole.
#: With a second Galaxy architecture that reading is wrong, and it was wrong in
#: a load-bearing way: `validate_galaxy_mesh` and the two allowlists in
#: `ccl.py` and `resources.py` are keyed on `supported_galaxy_architectures()`,
#: so a module asking "what architecture is Galaxy" has to be asking about a
#: mesh. Use `mesh_device.arch()`, or `galaxy_topology(mesh_device)`.
WORMHOLE_GALAXY_ARCHITECTURE = WORMHOLE_GALAXY_TOPOLOGY.architecture
...
def lm_head_reduce_core_count(
    ...
    **The reduction may not take the whole worker envelope.**
    ...
    LM head crashed the process the first time it ran.
...
def galaxy_padded_vocab_size(vocab_size: int) -> int:
    ...
    enough, and the extra condition is not cosmetic.
...
def dense_matmul_worker_rectangle(height: int, topology: GalaxyChipTopology | None = None) -> ttnn.CoreRangeSet:
    ...
    The search is what makes this architecture-neutral, and Blackhole is the
    case it was written for: with no prefetcher there is no sender column to
    split the envelope, so `worker_cores()` is the single rectangle
    ``x=1..10, y=0..9`` and the search returns all ten columns rather than
    three. Nothing here names either number.
    """
...
#: it. This sits between the two, so every already-qualified shape keeps the
...
def dense_matmul_output_blocks(per_core_M: int, per_core_N: int, in0_block_w: int) -> tuple[int, int]:
    ...
    - measured byte-identically on both models at
...
def dense_matmul_program_config(
    rows: int, local_k: int, local_n: int, topology: GalaxyChipTopology | None = None
) -> Any:
    ...
    This limit is terminal for exactly this ``(7, 1)`` grid. The recipes moved
    the *MLP* to the ring/``gather_in0`` form but left both attention matmuls on
    this dense config, so the limit was still live on first silicon. ``ttnn`` grew
    ...
    allowed = dense_matmul_worker_rectangle(grid_y, topology)
...
def worker_matmul_rectangle(topology: GalaxyChipTopology | None = None) -> ttnn.CoreRangeSet:
    ...
    return dense_matmul_worker_rectangle(worker_cores(topology).bounding_box().grid_size().y, topology)
...
def column_user_selector_program_config(
    users_per_column: int, padded_local_vocab: int, topology: GalaxyChipTopology | None = None
) -> Any:
    ...
    - **it must not resolve its own grid.** Left to auto-selection the op takes
      ...
      (reproduced in three fresh processes on both models);
    ...
    So this is the 1D ``mcast_in0`` form over the full worker rectangle, with
    ``per_core_N`` rounded up to a divisor of the tile count so the last core
    carries no remainder and ``out_subblock_w`` divides it. That rectangle is 30
    cores on Wormhole and 100 on Blackhole, which is why the count is read from
    the topology rather than written here -- this used to resolve the Wormhole
    rectangle for any mesh, silently, exactly like the placement resolvers did.
    """
    ...
    allowed = worker_matmul_rectangle(topology)
...
def sdpa_program_config(
    ...
    *,
    ...
    The long-sequence ``k_chunk_size`` comes off the descriptor rather than being
    named here: it is 512 on Wormhole and 256 on Blackhole, which is the one
    chunk size the reference splits by architecture. ``q_chunk_size`` is 256 on
    both, and the sub-2048 pair is 64/64 on both, so neither is parameterized.
    """
    ...
    if decode:
        ...
        q_chunk_size, k_chunk_size = 256, resolved.prefill_sdpa_k_chunk
...
def distributed_norm_decode_memory_config(
    ...
    there and binds it to the stats tensor's L1 address (enforced by
    ``RMSNorm2D._require_fused_stats_placement``). Derive it with
...
def distributed_norm_stats_memory_config(residual_memcfg: ttnn.MemoryConfig) -> ttnn.MemoryConfig:
    ...
    is why `RMSNorm2D._require_fused_stats_placement` rejects a mismatch
    outright. Deriving it here is what keeps the persistent
...
def resolve_galaxy_decode_placements(geometry: GalaxyDenseGeometry, mesh_device: Any) -> GalaxyDecodePlacements:
    """Resolve every decode placement the qualified recipes use.

    **The topology comes from the mesh, and on Blackhole this raises.** That is
    the intended behaviour and it is a fix, not a limitation: this function
    used to take a `mesh_device`, validate it -- which accepts Blackhole -- and
    then resolve every core set from the *Wormhole* default, because none of
    the helpers were passed a topology. A Blackhole caller got Wormhole core
    coordinates on a 12x10 grid with nothing raised, which is the silent class
    this port keeps paying for.

    Now it fails at the first ring access, with `_require_ring` naming
    `capabilities.has_ring_matmul`. The decode recipe is built around the
    24-core `gather_in0` ring for the MLP and the LM head, and Blackhole
    milestone 1 has no ring at all -- so a *correct* Blackhole decode placement
    set is a different recipe, not a re-parametrization of this one. Writing it
    is model bring-up (phase 6); the module suites build their own decode
    recipes, which is why phase 5 does not need it.

    `resolve_galaxy_prefill_placements` has no ring and therefore does resolve
    on Blackhole.
    """

    topology = galaxy_topology(mesh_device)
    workers = worker_cores(topology)
    ring = ring_cores(topology)
    receivers = ring_receiver_cores(topology)

    qkv_core_count = geometry.local_qkv_size // geometry.head_dim
    qkv_cores = _subgrid_cores(qkv_core_count, row_wise=False, topology=topology)
    head_cores = _subgrid_cores(GALAXY_PHYSICAL_BATCH, row_wise=False, topology=topology)
    sdpa_cores = _subgrid_cores(GALAXY_PHYSICAL_BATCH, row_wise=True, topology=topology)
    gather_user_cores = _subgrid_cores(GALAXY_PHYSICAL_BATCH, row_wise=True, topology=topology)
    kv_cores = _subgrid_cores(geometry.users_per_column, row_wise=False, topology=topology)
    sdpa_output_cores = _subgrid_cores(geometry.users_per_column, row_wise=True, topology=topology)
    ...
    # LM head's ring; the earlier recipes simply had not wired it up, so the
    ...
    lm_head_reduce_cores = _subgrid_cores(
        lm_head_reduce_core_count(padded_local_vocab, workers.num_cores()), row_wise=True, topology=topology
    )
    # Placements carry the padded physical width; resource keys carry the
    # logical width TTNN reports for the same tensor.
    reduce_scatter_cores = _subgrid_cores(
        geometry.decode_reduce_scatter_padded_width // TILE, row_wise=True, topology=topology
    )
    ...
    return GalaxyDecodePlacements(
        ...
        attention_qkv_program_config=dense_matmul_program_config(
            GALAXY_PHYSICAL_BATCH, geometry.local_dim, geometry.local_qkv_size, topology
        ),
        attention_wo_program_config=dense_matmul_program_config(
            GALAXY_PHYSICAL_BATCH, geometry.local_attention_dim, geometry.local_dim, topology
        ),
        attention_sdpa_program_config=sdpa_program_config(
            geometry.max_seq_len, decode=True, sub_core_grids=sdpa_cores, topology=topology
        ),
        ...
        mlp_w1_w3_program_config=ring_matmul_program_config(geometry.local_dim, padded_local_hidden, topology=topology),
        mlp_w2_program_config=ring_matmul_program_config(
            geometry.local_hidden_dim, padded_local_dim, topology=topology
        ),
        mlp_reduce_scatter_memcfg=ttnn.MemoryConfig(
            ttnn.TensorMemoryLayout.WIDTH_SHARDED,
            ttnn.BufferType.L1,
        ...
        lm_head_program_config=ring_matmul_program_config(
            geometry.local_dim, padded_local_vocab, global_cb_receivers=1, topology=topology
        ),
...
def resolve_galaxy_prefill_placements(geometry: GalaxyDenseGeometry, mesh_device: Any) -> GalaxyPrefillPlacements:
    ...
    **This one does resolve on Blackhole**, unlike its decode counterpart. The
    prefill recipe is already DRAM-interleaved and already uses the dense
    multicast matmul rather than the 24-core ring, so there is nothing here
    that milestone 1 lacks -- only geometry, which the topology supplies. The
    resolved worker envelope and every matmul rectangle inside it therefore
    come out as Blackhole's ten columns rather than Wormhole's three.

    It used to resolve Wormhole's regardless, silently, because none of these
    helpers was passed a topology.
    """
    ...
    return GalaxyPrefillPlacements(
        ...
        attention_program_configs={
            length: dense_matmul_program_config(length, geometry.local_dim, geometry.local_qkv_size, topology)
            for length in geometry.prefill_sequence_lengths
        },
        attention_wo_program_configs={
            length: dense_matmul_program_config(length, geometry.local_attention_dim, geometry.local_dim, topology)
            for length in geometry.prefill_sequence_lengths
        },
        attention_sdpa_program_configs={
            length: sdpa_program_config(length, decode=False, sub_core_grids=workers, topology=topology)
            for length in geometry.prefill_sequence_lengths
        },
        batched_attention_program_configs={
            length: dense_matmul_program_config(
                GALAXY_PHYSICAL_BATCH * length, geometry.local_dim, geometry.local_qkv_size, topology
            )
            for length in geometry.batched_prefill_sequence_lengths
        },
        batched_attention_wo_program_configs={
            length: dense_matmul_program_config(
                GALAXY_PHYSICAL_BATCH * length, geometry.local_attention_dim, geometry.local_dim, topology
            )
            for length in geometry.batched_prefill_sequence_lengths
        },
        ...
        chunked_sdpa_program_config=chunked_sdpa_program_config(
            sub_core_grids=workers, chunk_alignment=geometry.chunk_alignment, topology=topology
        ),
    )
...
def rope_core_grids(mesh_device: Any, *, use_qk_fused: bool) -> tuple[Any, ttnn.CoreRangeSet]:
    ...
    which is the same class of defect as the fused-norm statistics placement: a
    grid named independently of the partition that has to contain it.
    ``_subgrid_cores`` is the qualified helper the attention KV, SDPA
    and reduce-scatter placements already use; it anchors at the first worker
    core ``(1, 0)`` and never leaves ``worker_cores()``.

    The topology comes from the mesh rather than the Wormhole default, because
    the anchor `_subgrid_cores` uses is per-architecture: `(1, 0)` on Wormhole
    and on Blackhole it is `BLACKHOLE_FIRST_WORKER_COLUMN`. Resolving it from
    `mesh_device` is what stops a Blackhole RoPE setup from placing its decode
    cos/sin shards using Wormhole's envelope -- which is this exact `TT_FATAL`
    again, on the other architecture.
    """

    topology = galaxy_topology(mesh_device)
    core_grid = mesh_device.compute_with_storage_grid_size()
    rows = GALAXY_USERS_PER_COLUMN * (2 if use_qk_fused else 1)
    batch_grid = _subgrid_cores(rows, row_wise=True, topology=topology)
