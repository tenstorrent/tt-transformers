...
recipes issue: the MLP2D axis-1 reduce-scatter/all-gather pair and
...
from tt_transformers.models.galaxy.ccl import GalaxyMode
...
#: Per-core width of the shared axis-0 all-reduce scratch, in elements.
#:
#: The buffer is width-sharded over the worker envelope with a `(TILE, 1024)`
#: shard (`recipes.all_reduce_buffer_memcfg`), so the tensor's own width must be
#: `worker_cores * 1024` or the tensor and the spec that shards it disagree.
#: This used to be spelled `50 * 1024`, which is **Wormhole's** 50 worker cores
#: times this width -- a `worker_cores`-derived quantity written as a literal,
#: and so a B8-class architecture-pinned constant that B8 never listed. Blackhole
#: has **100** worker cores, so the same literal is half the width its own shard
#: spec implies. Derived from the memory config at the call site now, which makes
#: the two self-consistent by construction rather than by agreement.
_ALL_REDUCE_SHARD_WIDTH = 1024

#: Only a fallback, for a stand-in memory config with no shard spec: the width
#: the old literal encoded. Never used on a real mesh, where the spec answers.
_WORMHOLE_WORKER_CORES = 50


def _width_sharded_tensor_width(memory_config: Any, fallback_cores: int) -> int:
    """Return the total width a width-sharded spec implies: cores x shard width.

    Reads the shard spec rather than trusting a constant, and falls back to
    `fallback_cores * _ALL_REDUCE_SHARD_WIDTH` when a stand-in memory config
    carries no shard spec -- host tests build these plans out of
    `SimpleNamespace` objects, and a plan builder that raised on one would break
    every such test for no gain.
    """

    shard_spec = getattr(memory_config, "shard_spec", None)
    if shard_spec is None:
        return fallback_cores * _ALL_REDUCE_SHARD_WIDTH
    grid = getattr(shard_spec, "grid", None)
    shape = getattr(shard_spec, "shape", None)
    if grid is None or shape is None or not hasattr(grid, "num_cores"):
        return fallback_cores * _ALL_REDUCE_SHARD_WIDTH
    return int(grid.num_cores()) * int(shape[1])
...
def build_galaxy_decode_collectives(
    ...
    *,
    ...
    output_all_reduce = GalaxyCollectivePlan(
        ...
        persistent_output_specs=(
            _spec(
                # Width read off the spec that shards it, not named: 50 worker
                # cores on Wormhole and 100 on Blackhole, so the old `50 * 1024`
                # literal was Wormhole's core count in disguise.
                (
                    *GALAXY_MESH_SHAPE,
                    TILE,
                    _width_sharded_tensor_width(placements.all_reduce_buffer_memcfg, _WORMHOLE_WORKER_CORES),
                ),
    ...
    lm_head_all_reduce = GalaxyCollectivePlan(
        ...
        #
        # It used to be built inside the graph body, as an
        # `interleaved_to_sharded` copy of the DRAM buffer below, and freed in the
        # same `finally` (`collectives.py`). Eagerly that is safe - the host
        # barrier at the same site orders the peers before the free, and the next
        # step allocates a fresh one. Under a trace it is not: the free is host-side
        # allocator bookkeeping that the captured program cannot see, so the
        # captured LM head keeps the address baked in as a **barrier-less fabric
        # write target** - `all_reduce_async`'s `buffer_tensor` overload takes no
        # barrier semaphore (`all_reduce_async.hpp:51-64`) - and on every replay
        # whatever the allocator has since put there is written by skewed peers.
        #
        # Measured, at 80 layers, one process each:
        #   * every decode
        #     plan buffer, including the final norm's stats, is bit-identical
        #     across replays and against eager, while the logits differ by up to
        #     17 - so the divergence is born after the final norm, and this is the
        #     only op left in that interval that destroys a fabric-write target
        #     inside the capture body;
        #   * with the buffer
        #     allocated once outside any capture region and never freed, the
        #     traced decode is `pcc>=0.999 in 14 of 14`, `exact vs eager in 14`,
        #     `13 of 13 replays bit-identical`. Against the same tree's control
        #     (`p4`: 3 of 14, 0 of 13) that is the whole of coverage item 4.
        #
        # It costs 52 224 B of L1 per `lm_head_reduce_core` on Llama-3.3-70B
        # (42 cores x 48 tiles x 1088 B at bfloat8_b) and 65 280 B on Qwen3-32B,
        # resident. `2d464ea53dc` estimated ~126 kB and read that as infeasible;
        # that is the **DRAM** buffer's size at bfloat16, and this is its bfloat8_b
        # L1 copy. The reserve above the global circular buffer takes it:
        # the decode graph's own demand on that reserve measures
        # 55 648 B of the 65 536 `GALAXY_GLOBAL_CB_HEADROOM`
        # leaves, and a plan buffer is allocated *before* the global CB, so this
        # one joins the top-packed region and moves the CB down by its own size
        # rather than competing for the gap.
        #
        # It is an intermediate rather than a second persistent output because
        # `all_reduce_async` never returns it: it is pure scratch
        # (`all_reduce_async_device_operation.cpp`), fully overwritten by the ring,
        # and `p8` confirms on silicon that reusing it across calls without
        # re-zeroing is bit-identical to the fresh zeroed copy it replaces.
        intermediate_output_specs=(
            _spec(
                (*GALAXY_MESH_SHAPE, TILE, geometry_padded_local_vocab * GALAXY_COLUMNS),
                placements.lm_head_all_reduce_buffer_memcfg,
                dtype=lm_head_dtype,
                mesh_mapper=row_shard,
            ),
        ),
