...
That is defect **F-1** in `tttv2_milestone_c_evidence/trace/REPORT.md`, measured
on silicon at 80 Llama layers with 40 traced prefill replays per arm:

| arm | replays matching eager | bit-identical to eager |
| --- | --- | --- |
| `j1` unmodified tree | 18 of 40 | 10 of 40 |
| `b4` every capture-body DRAM free suppressed, held through the replays | 40 of 40 | 37 of 40 |
| `b6` the same, but the whole hold **released before the replays** | **40 of 40** | **40 of 40** |
...
class CaptureFreeDeferral:
    ...
    def flush(self) -> int:
        ...
        if held:
            ...
            # hold engaged at all, and how much it cost. Every arm of the F-1
