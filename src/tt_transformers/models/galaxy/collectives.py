...
not merged into ``src/tt_transformers/modules/tt_ccl.py``; see the CCL follow-up note
...
class GalaxyAttentionCollectives:
    ...
    def gather_users(self, tensor: Any, *, mode: str, **_: Any) -> Any:
        ...
        **L1**; `CaptureFreeDeferral` defers DRAM frees inside a capture body and
        deliberately not L1 (attempt 6's `b1`: a prefill program's static
        ...
        once per layer rather than once per graph. The silicon, all in
        `tttv2_milestone_c_evidence/trace/logs/`, all Qwen at 64 layers in the
        four-capture-body device-sampling configuration, `observed (1 run)` each:

        | arm | eager-vs-traced logits pairs differing, of 10 | worst delta | sampled tokens pairs differing, of 10 |
        | --- | --- | --- | --- |
        | unmodified (`q3`) | 10 | 713 031 680 | 5 |
        | sampler gathers made exclusive instead (`qC`) | 10 | 2.0625 | 6 |
        | **this change (`qA`)** | **4** | **0.375** | **0** |
        | both (`qB`) | 3 | 0.375 | 0 |

        `q8_Q_logits_trace_alone.log` is the defect on its own: ten back-to-back
        traced decodes with no sampler anywhere, **0 of 10 bit-identical to eager
        and 0 of 9 to each other**, against the same trace in the
        three-capture-body configuration at 14 of 14 exact (`p8`).
