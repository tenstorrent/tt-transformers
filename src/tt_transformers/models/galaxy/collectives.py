...
This is the deliberately separate Galaxy CCL owner. It is not merged into
``src/tt_transformers/modules/tt_ccl.py``; merging the two is open follow-up
work.
"""
...
from tt_transformers.modules.attention.attention_2d import Attention2DLowLevelCallables, PrefillRowMode
...
def _ccl_shape_note(name: str, tensor: Any) -> str:
    ...
    core waiting forever, with no abort and no traceback. Printing both side by
    side makes that visible at a glance instead of after a gdb dump: the line
    that first exposed it read `tiles=501 spec_tiles=504`.
    ...
    def _persistent_all_reduce(self, tensor: Any) -> Any:
        ...
        * **The result is placed back into the caller's placement.** `LMHead2D`
        ...
        def trace_step(message: str) -> None:
            ...
            Enqueues are asynchronous, so naming the three ops was not enough:
            all three printed and the block landed on the collective's own final
        ...
        if not resource.intermediate_output_buffers:
            raise ValueError(
                "decode LM head all-reduce requires the plan's L1 buffer intermediate; see "
                "plans.py::build_galaxy_decode_collectives at lm_head_all_reduce"
            )
        key = resource.key
        reduced = None
        # The plan's own L1 buffer, allocated once for the life of the process and
        # **not** freed here.
        #
        # This used to be `ttnn.interleaved_to_sharded(persistent_output_buffers[0],
        # ...)` - a fresh L1 copy of the DRAM buffer made inside the graph body -
        # freed in the `finally` below, to keep the decode step's largest
        # allocation off the worker cores for all but one op. Eagerly that is safe:
        # the `synchronize` in the same `finally` orders the peers' fabric writes
        # before the free, and the next step allocates a fresh one.
        #
        # Under a trace it is not safe, and it is the whole of coverage item 4.
        # `resources.synchronize("decode")` is a **no-op while capturing**
        # (`resources.py`), and the free is host-side allocator bookkeeping the
        # captured program cannot see - so the captured LM head keeps this address
        # baked in as a fabric write target with no barrier semaphore
        # (`all_reduce_async`'s `buffer_tensor` overload has no such parameter,
        # `all_reduce_async.hpp:51-64`), and every replay lets skewed peers write
        # over whatever the allocator has since put there. The measurements, the
        # cost and why 42 cores' worth of bfloat8_b is affordable are recorded at
        # the plan, beside the spec.
        _ccl_trace("lm_head buffer from the plan")
        buffer_l1 = resource.intermediate_output_buffers[0]
        ...
        try:
            ...
            # `buffer_l1` is deliberately **not** deallocated: it is plan-owned and
            # released by `GalaxyResources.cleanup`, after every trace is released.
            # Freeing it here is the defect this comment's neighbour describes.
...
def compose_galaxy_sampled_tokens(tensor: Any, *, mesh_device: Any = None, users: int | None = None) -> torch.Tensor:
    ...
    Measured on `(8, 4)` with Qwen3-32B, two fresh processes, byte-identical::
        ...
        [probe] greedy tokens: [265, 2631, 1916, 220, 17, 15, 17, 17,
                                265, 2631, 1916, 220, 17, 15, 17, 17,
                                265, 2631, 1916, 220, 17, 15, 17, 17,
                                265, 2631, 1916, 220, 17, 15, 17, 17]
        [probe] greedy agrees with host argmax in 7/32 slots

    That is a readback defect, not a sampling one: the eight users it does
    surface were right. Composing by distribution - the four
...
class GalaxyColumnUserSelector:
    ...
    - output ``[1, 1, 8, local_vocab]``, which is exactly the layout the
      ``Sampling2D`` hardware qualification staged by hand.

    **Two things the matmul needs that the caller does not supply**, both
    measured on silicon at three fresh processes on both models:

    - **A sharded ``in1``.** ``LMHead2D.decode_forward`` returns its output under
    ...
    - **A sub-device grid mismatch.** With ``in1`` interleaved the *same line* then aborts under the
      ...
      :func:`~tt_transformers.models.galaxy.recipes.column_user_selector_program_config`.
      ...
      exactly how this reached silicon.

    **Math fidelity.** The selection is a matmul, and a matmul is not a copy. With
    ...
    ``tests/models/galaxy/test_column_user_selector_wh_galaxy.py``,
    ...
    def __init__(
        ...
        *,
        ...
        # Exact by default: see the math-fidelity note above. `None` here is a lossy gather.
...
class GalaxyAttentionCollectives:
    ...
    def __init__(
        ...
        *,
        ...
        #: Resolved once by `_plan_owned_addresses`; the plan's buffers are
        #: allocated before any graph runs and never freed.
        self._plan_addresses: frozenset[int] | None = None
        self._closed = False
    ...
    def cleanup(self) -> None:
        ...
        # NOT dropping `_plan_addresses` here is deliberate, and analysed: the
        # 2026-09-04 advisory §7.6 asks for it as hygiene on `c1f30128e13`, and
        # it is a correct one-line change, but it is a *behaviour* change to
        # shared Galaxy code with no measured benefit. `cleanup()` is reached
        # only from `model.close()`, after all serving; `_plan_addresses` is read
        # only by `is_borrowed_output`, called only from
        # `Attention2D._transition` inside a graph body; and no graph body runs
        # after `model.close()` - coverage item 9's three executor cycles share
        # one model and close it once, at the end. So the line is unreachable on
        # every path measured, and applying it would put the fifteen-node Llama
        # gate measured at `c1f30128e13` at a commit that is no longer HEAD.
        # Apply it in the next job that re-measures the gate for its own reasons.
        self._closed = True
    ...
    def is_borrowed_output(self, tensor: Any) -> bool:
        """Report whether a collective result aliases a buffer the plan owns.

        `Attention2D` frees every stage result it is not told is borrowed
        (`attention_2d.py::_transition`), so this seam decides whether a
        plan-owned buffer survives the graph body. It used to be hard-wired to
        `False`, which was safe only while nothing here returned one - the
        decode user gather now does, and freeing that buffer is the defect
        `gather_users` above exists to close.

        Identity is not enough: `ttnn` hands back a fresh Python wrapper over the
        same buffer, which is the trap `rmsnorm_2d.py:391-406` documents. This
        compares `buffer_address()`, the same rule `_aliases_borrowed_buffer`
        uses at its one existing call site. The plan's buffers are allocated in
        `GalaxyCCL.__init__` and never freed, so their addresses are fixed for
        the life of the process and the set is computed once.

        **The set spans both modes' plan buffers, deliberately.** A caller asks
        this about a tensor, not about a mode, and the only wrong answer that
        costs anything is `False` for a buffer the plan owns - that frees a
        fabric-write target. Prefill and decode plans are provisioned by the same
        `GalaxyResources` owner with disjoint address ranges, so spanning both
        cannot turn a decode-owned address into a false positive; it only closes
        the case where a prefill-owned buffer reaches a decode-side release. The
        cost is one extra walk of a registry, once per process.
        """

        address = _buffer_address(tensor)
        return address is not None and address in self._plan_owned_addresses()

    def _plan_owned_addresses(self) -> frozenset[int]:
        """Every address the resource owner lends out, resolved once."""

        if self._plan_addresses is None:
            addresses: set[int] = set()
            for mode in ("decode", "prefill"):
                try:
                    context = self.resources.context(mode)
                except (KeyError, ValueError):
                    # A caller may configure one mode only; the other lends
                    # nothing. Anything else is a wiring fault and stays loud.
                    continue
                for key in getattr(context, "resource_keys", ()) or ():
                    resource = context.resources(key.operation, key.cluster_axis, key.geometry, key.sequence_key)
                    for buffer in (
                        *(getattr(resource, "persistent_output_buffers", ()) or ()),
                        *(getattr(resource, "intermediate_output_buffers", ()) or ()),
                    ):
                        address = _buffer_address(buffer)
                        if address is not None:
                            addresses.add(address)
            self._plan_addresses = frozenset(addresses)
        return self._plan_addresses
    ...
    def gather_users(self, tensor: Any, *, mode: str, **_: Any) -> Any:
        """Gather the per-column user group into the plan's own destination.

        The destination is what the three peer column devices **fabric-write
        into**, and `ttnn.all_gather` states the contract for it in its own
        docstring: *"Pre-allocated output tensor... This must be allocated before
        invoking any op to avoid races."* Letting the op allocate its own
        satisfied neither half. `attention_gather_users_memcfg` is
        `height_sharded_memory_config(...)` (`recipes.py`), so the buffer is in
        **L1**; `CaptureFreeDeferral` defers DRAM frees inside a capture body and
        deliberately not L1 (a prefill program's static
        circular buffers need the L1 the previous program frees); and
        `Attention2D._transition` freed the result in the same graph body, once
        per layer. Eagerly the barrier below orders the peer writes before the
        free. Under a trace capture the barrier is a no-op
        (`GalaxyResources._synchronize`) and the free is host-side allocator
        bookkeeping the captured program cannot see, so the captured gather keeps
        a **barrier-less fabric write target baked in at an address the allocator
        has given away** - and every replay lets skewed peers write over whatever
        now owns it.

        That is `9f5769cf2e4`'s LM-head defect, one op earlier in the graph and
        once per layer rather than once per graph. Measured on silicon, all Qwen
        at 64 layers in the four-capture-body device-sampling configuration, one
        run each:

        | arm | eager-vs-traced logits pairs differing, of 10 | worst delta | sampled tokens pairs differing, of 10 |
        | --- | --- | --- | --- |
        | unmodified | 10 | 713 031 680 | 5 |
        | sampler gathers made exclusive instead | 10 | 2.0625 | 6 |
        | **this change** | **4** | **0.375** | **0** |
        | both | 3 | 0.375 | 0 |

        The defect also reproduces on its own: ten back-to-back traced decodes
        with no sampler anywhere, **0 of 10 bit-identical to eager and 0 of 9 to
        each other**, against the same trace in the three-capture-body
        configuration at 14 of 14 exact.

        **It costs nothing.** The plan has always provisioned this buffer for
        this key - `plans.py::gather_users`, spec
        `(1, batch, local_heads, head_dim)` - and until now no caller borrowed
        it, so it was allocated and never used. Taking it removes one allocation
        per layer rather than adding one. `is_borrowed_output` is what stops
        `Attention2D` freeing it.

        **READ THIS BEFORE CHANGING THE DECODE GRAPH'S COLLECTIVE ORDER.**
        Supplying `output_tensor` compiles the op's own start barrier out -
        `all_gather_unicast_factory.cpp`: `do_init_barrier =
        !persistent_output_tensor.has_value()` - which is the barrier-versus-
        address-exclusivity trade `CCL_Performance_Best_Practices.md` documents,
        and the two defenses are mutually exclusive. Exclusivity is what makes it
        safe here, and it is only sufficient because **consecutive uses of this
        one shared buffer are separated by an axis-1 collective**: layer N's
        gather and layer N+1's are separated by `reduce_create_qkv_heads`, which
        no row peer can complete before every row peer has *started* it, i.e. is
        past `nlp_concat_heads_decode(N)`. There is exactly one buffer per key
        for all 64-80 layers, so if a future graph change removes the axis-1
        collective between two `gather_users` calls, layer N+1's barrier-less
        peer writes can land in layer N's consumer and **nothing in this runtime
        would report it**. Add the barrier back (drop `output_tensor`) or
        double-buffer the key, the way `models/demos/llama3_70b_galaxy`'s
        `llama_ccl.py` does with `num_cbs = 2`.
        """
        ...
        output = ttnn.all_gather(
            ...
            output_tensor=resource.persistent_output_buffers[0],
        )
        ...
        **Prefill** keeps the plain ``reduce_scatter`` + ``all_gather`` pair,
        ...
        if mode != "decode":
            reduced = ttnn.reduce_scatter(
                ...
                # This op offsets its worker walk by the partition's own origin, so on a
                # partition that does not start at the grid origin the count it picks for
                # itself walks off the far column and aborts. `None` on every Wormhole
                # partition, so the qualified path is unchanged.
