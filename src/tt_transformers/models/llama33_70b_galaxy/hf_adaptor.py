...
from tt_transformers.models.galaxy.recipes import GALAXY_PHYSICAL_BATCH, validate_galaxy_mesh
from tt_transformers.models.llama33_70b_galaxy import weight_utils
...
#: Cache-identity topology segment. The shared cache policy requires exactly one
#: path segment and appends it once, which reproduces the `galaxy_8x4` segment the
#: tt-metal original built by hand.
GALAXY_TOPOLOGY = "galaxy_8x4"


def _local_files_only() -> bool:
    # Deliberately `offline_mode()` and not the tt-metal predicate this replaced.
    # That one also treated `CI` as offline; the shared policy states that "CI
    # alone never selects offline mode", so a CI run now needs an explicit
    # `TT_TRANSFORMERS_OFFLINE`, `HF_HUB_OFFLINE` or `TRANSFORMERS_OFFLINE`.
    return offline_mode()
...
def _cache_path(
    ...
    *,
    ...
    # The tt-metal original resolved `TT_CACHE_PATH` itself and fell back to a
    # CWD-relative `model_cache/<hf_model>/galaxy_8x4`. Both are forbidden here:
    # cache identity is owned by the shared policy, which folds the topology in
    # exactly once and records the full identity inputs in the preflight report.
    resolution = resolve_model_cache(
        hf_model_id=hf_model,
        hf_revision=hf_revision,
        topology=GALAXY_TOPOLOGY,
        cache_dir=cache_dir,
...
def from_pretrained(
    ...
    *,
    ...
    # `src/tt_transformers/models/galaxy/prefetch.py` for the arithmetic.
    ...
    ``n_layers`` builds a layer subset, which is how the one-layer numerical
    qualification runs against the real checkpoint.
    """

    validate_galaxy_mesh("Galaxy Llama-3.3-70B", mesh_device)
    # The tt-metal original also called `ttnn.SetDefaultDevice(mesh_device)` here.
    # Mutating TTNN's process-global default is reserved to
    # `tt_transformers.device_ownership` in this package, and nothing in the Galaxy
    # build path reads it: `model.py` threads `mesh_device` explicitly throughout.
    # NOT RE-VERIFIED ON A MESH — see the port's Phase 6 notes.
    ...
    cache_path = _cache_path(hf_model, mesh_device, cache_dir, dtype=precision.kv_cache_dtype)
