...
#: ring. The attention decode projections do **not**: a terminal grid limit
...
#: registered, both are correct.
#:
#: Moving the attention decode matmuls onto the ring would let them be
#: prefetched again and is the performance follow-up. Until
...
class Llama33_70BGalaxyPrecision:
    ...
    Defaults reproduce the dtypes the WH Galaxy module qualifications used for
    this geometry.
    """
...
class Llama33_70BGalaxyModelParameters:
    ...
    def __post_init__(self) -> None:
        ...
        if not 1 <= self.n_layers <= LLAMA33_70B_CHECKPOINT_CONTRACT["num_hidden_layers"]:
            raise ValueError(
                f"n_layers must be in [1, {LLAMA33_70B_CHECKPOINT_CONTRACT['num_hidden_layers']}], got {self.n_layers}"
            )
...
def build_llama33_70b_galaxy_transformer_2d_config(
    *,
    ...
    #
    # Measured on `(8, 4)` with the non-fused pair: the decode step wrote
    ...
    * ``to_memory_config(t, memcfg)`` between two shard specs that differ in both
    ...
    if source_memcfg.is_sharded():
        if not target_memcfg.is_sharded():
            ...
            #     TT_FATAL ... Kernel group cores do not match sub device cores
            #                  for programmable core type TENSIX
            # The bug was latent for every interleaved target
