...
"""Model-owned Qwen3-32B Galaxy `(8, 4)` execution composition and cleanup root.

This is the model-owned executor for the reconstructed 2D tensor model. It composes
the common runtime (`src/tt_transformers/llm_runtime`) exactly as
`src/tt_transformers/models/qwen3_32b/executor.py` composes it for the 1D model, and it
is the resource and cleanup root for one execution lane.

**This file is the reuse measurement, so read what it is not.** It is
`src/tt_transformers/models/llama33_70b_galaxy/executor.py` with four identifiers renamed
and this docstring rewritten — no model-family branch, no Qwen-shaped special case,
no shared-code change of any kind. Everything that differs between the two models is
reached through the model package and the frozen configuration it resolves:

* **Q/K norm.** `Attention2D` is built with per-head Q/K `RMSNorm2D` geometry for
  Qwen3 and without it for Llama-3.3. That is `_build_block_config`'s decision in
  `src/tt_transformers/models/qwen3_32b_galaxy/model.py`, resolved before the executor
  exists. It is *head-local* normalisation, not hidden-dimension distributed norm —
  confusing the two is a specific recorded hazard — and
  the executor never touches either: it hands the graph a hidden state and takes one
  back.
* **Geometry.** hidden 5120, intermediate 25600, head_dim 128, vocab 151936, 64
  heads decoupled from `dim` (`Qwen3_32BGalaxyModelParameters.attention_dim`), 64
  layers. Every value the executor reads comes from `model.geometry`,
  `model.config` or `model.kv_specs`; none is written down here.
* **Vocabulary.** 151936 against the other model's 128256, both padded by
  `galaxy_padded_vocab_size` and both composed by `collectives.compose_galaxy_logits`,
  which takes the size as an argument.

Two things make this executor longer than its 1D sibling, and both of them are
*model-owned adaptation* rather than new mechanics. They are the same two the
`llama33_70b_galaxy` executor documents, and they are why the file is this length in
both packages.
...
`Qwen3_32BGalaxyTransformer2D` exposes a different, mode-explicit contract
...
Galaxy, Qwen, 2D-mesh or `(8, 4)` knowledge. See `_GalaxyRuntimeModelView`.

**2. Two placements differ from the 1D convention, and both were qualified in
`GalaxyDirectRunner`.**
...
replicated over the four columns; composing it along the wrong axis is a
measured defect, and `collectives.compose_galaxy_logits` is the qualified
composition that carries the measurement. This view composes with it. Decode returns the composed
...
**The warmup prefix.** `WarmupConfig.cached_prefill_tokens` is the one common-runtime
field the Llama executor had to add, and this executor consumes it exactly as its
sibling does: from `self.geometry.chunk_alignment`, which is the model's own property.
Adding it was that work's cost; using it here costs nothing, which is the extension
discipline working as designed.

**Out of scope here, deliberately.** No `generator.py`, no vLLM adapter, no lane
group. Batched (concat-32) prefill is out of scope: this executor resolves
`supports_batched_prefill=False` and `post_process_batched_prefill_output` raises.
Tracing is a separate concern — the trace collaborators are constructed exactly as the
...
from tt_transformers.models.qwen3_32b_galaxy.model import Qwen3_32BGalaxyTransformer2D
...
class Qwen3_32BGalaxyExecutorConfig:
    ...
    #: This executor prefills one row at a time. It resolves the runtime's
...
class _GalaxySamplingView:
    ...
    **Keyword names.** `DecodeRuntime._sample_device` calls
    ...
    silicon, which is why no evidence for device-sampled decode *through an
    executor* existed before this view.
...
class _GalaxyRuntimeModelView:
    ...
    Every method here delegates to `Qwen3_32BGalaxyTransformer2D`; the view owns
    ...
    def __init__(self, model: Qwen3_32BGalaxyTransformer2D, *, device_sampling_enabled: bool):
        self._model = model
    ...
    @property
    def model(self) -> Qwen3_32BGalaxyTransformer2D:
        return self._model
    ...
    def set_trace_active_probe(self, probe: Any) -> None:
        ...
        kills the *next* replay - a segmentation fault in
        `rms_norm_pre_all_gather` on the second of thirty-two prefills.
        ...
        **Without one it must**, and that is not a preference either: releasing
        ...
        That regression has been measured: the executor's own prefill claim,
        argmax 1709 against a freshly recomputed reference's 8703, with logits
        of magnitude 2.9e14.
    ...
    def prepare_prefill_rot_mats(self, position_indices: Any) -> list[Any]:
        ...
        table copy and is the qualified path; the runtime supplies the
        ...
        **A device-side gather was tried first and is not equivalent**, measured
        on the sibling Galaxy model and inherited here rather than re-measured. `ttnn.embedding` over the same table needs no readback and
        would be trace-compatible, but it moved the KV that prefill writes: with
        the gather, the first layer's K measured PCC 0.907 against the same request through
        `GalaxyDirectRunner` while the logits agreed at 0.9994 — and K is the one
        tensor of the pair that passes through RoPE. The rotary is shared 2D-module
        code (`RotarySetup2D`), so the finding transfers; this executor therefore
        uses the qualified slice, and a four-byte read per prefill step is a
        smaller price than an unqualified rotary. **Note for tracing work: that
        readback is the one host round trip on the prefill path.**
        """
        ...
        *,
        ...
        if rows != 1 or len(user_ids) != 1:
            raise NotImplementedError(
                "batched (concat-32) prefill is not supported; this executor prefills one row at a time"
            )
        ...
        # A captured graph cannot carry a host
    ...
    def post_process_batched_prefill_output(self, *args: Any, **kwargs: Any) -> Any:
        raise NotImplementedError("batched (concat-32) prefill extraction is not supported for the Galaxy executor")
    ...
    def compose_decode_logits(self, value: Any) -> torch.Tensor:
        ...
        a measured defect. `compose_galaxy_logits` is the composition that
...
class Qwen3_32BGalaxyExecutor:
    """Compose every runtime owner for one Galaxy Qwen3-32B execution lane.

    Construction wires one `PrefillRuntime`, `DecodeRuntime`, `ProgramCompiler`
    and `EagerExecutor` over one full-mesh `Qwen3_32BGalaxyTransformer2D`, one
    ...
    def __init__(
        ...
        runtime_config: Any,
        config: Qwen3_32BGalaxyExecutorConfig,
    ) -> None:
        if not isinstance(config, Qwen3_32BGalaxyExecutorConfig):
            raise TypeError("config must be a Qwen3_32BGalaxyExecutorConfig")
        ...
        #               chunk_alignment            (attention_2d.py:908)
        #
        ...
        if config.trace.mode != "none":
            ...
            self.traced_executor = TracedExecutor(
                ...
                # sub device cores` - and that abort inside a
            ...
            # That is the observed `ttnn.end_trace_capture` hang.
            #
            ...
            # fix the traced-replay instability (non-finite traced decode logits
            # with the graph site selected), so enabling it
        ...
        self.warmup = WarmupCoordinator(
            ...
            # call. See `WarmupCoordinator.warmup_decode`.
    ...
    def configure_paged_kv_cache(self, config: PagedKVCacheConfig) -> None:
        ...
        than the construction ceiling is refused at binding —
            ...
            ValueError: paged KV cache shape must be (2048, 1, 32, 128),
                        got (95, 1, 32, 128)
            (measured on the sibling model; the module code that refuses is
             shared `Attention2D`)
    ...
    def warmup_model_prefill(
        ...
        *,
        ...
        try:
            # No DRAM address may be recycled inside a trace capture body.
    ...
    def warmup_model_decode(
        ...
        *,
        ...
        # The same capture-body free hazard as in `warmup_model_prefill`: see
        # `_capture_free_hold`.
        with self._capture_free_hold():
    ...
    def _capture_free_hold(self) -> Any:
        ...
        collective's fabric-write destination. The resource owner implements the
        hold, because it is the only thing that knows both
    ...
    def cleanup(self) -> None:
        ...
        if failures:
            _raise_cleanup_failures(failures, "Qwen3_32BGalaxyExecutor")
    ...
    def _ensure_active(self) -> None:
        if self._terminal:
            raise RuntimeError("Qwen3_32BGalaxyExecutor is terminal; construct a new executor")
        if self.prefill_runtime.transient_orphan_count or self.decode_runtime.transient_orphan_count:
            raise RuntimeError("Qwen3_32BGalaxyExecutor has unreleased transient resources; clean up this executor")


def build_qwen3_32b_galaxy_executor(
    llm: Any,
    config: Qwen3_32BGalaxyExecutorConfig,
) -> Qwen3_32BGalaxyExecutor:
    """Build one executor around an already-loaded Galaxy Qwen handle."""

    return Qwen3_32BGalaxyExecutor(llm.model, llm.runtime_config, config)
...
__all__ = [
    ...
    "build_qwen3_32b_galaxy_executor",
