...
* **Q/K norm.** `Attention2D` is built with per-head Q/K `RMSNorm2D` geometry for
  ...
  Milestone A's risk register names confusing the two as a specific hazard — and
...
field `c-exec-llama` had to add (D-C16), and this executor consumes it exactly as its
sibling does: from `self.geometry.chunk_alignment`, which is the model's own property.
Adding it was that job's cost; using it here costs nothing, which is the extension
...
class _GalaxyRuntimeModelView:
    ...
    def prepare_prefill_rot_mats(self, position_indices: Any) -> list[Any]:
        ...
        **A device-side gather was tried first and is not equivalent**, measured
        on the sibling Galaxy model by `c-exec-llama` and inherited here rather than
        re-measured. `ttnn.embedding` over the same table needs no readback and
        would be trace-compatible, but it moved the KV that prefill writes: with
        the gather,
        `tttv2_milestone_c_evidence/exec_llama/logs/i4_pagedkv_l1.log` reports the
        first layer's K at PCC 0.907 against the same request through
        ...
        smaller price than an unqualified rotary. **`c-trace`: that readback is the
        one host round trip on the prefill path.**
        """
    ...
    def configure_paged_kv_cache(self, config: PagedKVCacheConfig) -> None:
        ...
        than the construction ceiling is refused at binding —
            ...
            ValueError: paged KV cache shape must be (2048, 1, 32, 128),
                        got (95, 1, 32, 128)
            (`tttv2_milestone_c_evidence/exec_llama/logs/i9_shrink_l1.log`, the
             sibling model; the module code that refuses is shared `Attention2D`)
