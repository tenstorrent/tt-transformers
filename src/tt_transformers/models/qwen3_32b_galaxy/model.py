...
``src/tt_transformers/models/galaxy`` — the `(8, 4)` geometry and placement recipes,
...
from tt_transformers.models.galaxy.kv_contract import (
    GalaxyAttentionKVSpec,
    GalaxyPagedAttentionConfig,
    GalaxyPagedKVContract,
)
from tt_transformers.models.galaxy.plans import build_galaxy_resources_config, select_galaxy_resource
from tt_transformers.models.galaxy.prefetch import (
...
from tt_transformers.models.galaxy.resources import create_galaxy_resources
...
from tt_transformers.modules.embedding.embedding_2d import Embedding2D, Embedding2DConfig
...
from tt_transformers.modules.lm_head.lm_head_2d import LMHead2D, LMHead2DConfig
from tt_transformers.modules.mlp.mlp_2d import MLP2D, MLP2DConfig
...
from tt_transformers.modules.sampling.sampling_2d import Sampling2D, Sampling2DConfig
...
#: confined worker rectangle forced by a terminal grid limit, and a matmul
#: there cannot take its weight from a buffer whose receivers are the ring.
#:
...
#: entry meant for `wqkv`. Measured for Llama on `(8, 4)`: decode
...
class Qwen3_32BGalaxyPrecision:
    ...
    Defaults are the accuracy recipe. They reproduce the dtypes the WH Galaxy
    module qualifications used for this geometry, with one Qwen
...
class _UnprefetchedContext:
    ...
    defaults to sub-device *zero*, the prefetch senders - but they must
...
def _head_local_norm_config(
    ...
    **Decode must name the created heads' own placement, not interleaved DRAM.**
    ...
    own. This is the unresolved half of an earlier defect, whose own symptom was
    that head-local decode aborted in op validation before producing a
    numerical result; the fix for that made interleaved DRAM the *default*
...
def build_qwen3_32b_galaxy_transformer_2d_config(
    *,
    ...
    # fused one.
    #
    # Measured for Llama on `(8, 4)` with the non-fused pair: the decode
    ...
    embedding_config = Embedding2DConfig(
        ...
        # `embed_decode` a no-op instead of a second copy. This was found and
        # fixed for Llama first; this copy carried it unchanged.
    ...
    # column's user slice is an executor responsibility.
    ...
    sampling_config = (
        Sampling2DConfig(
            ...
            # The decode graph replicates the physical batch across the four
            # mesh columns and shards the vocabulary over the eight rows, so
            # every device is handed all 32 users. The sampler's
            # default placement shards the users too, and its vocabulary mask
            # then has 8 rows against the logits' 32. See `replicate_users`.
            replicate_users=True,
            # `decode_forward` is decode-only, so the decode plan's worker is the
            # right one. Without it the sampling gathers place themselves on
            # sub-device 0 and lose their ordering against `topk`.
            ccl_sub_device_id=lambda: resources.context("decode").worker_sub_device_id,
        )
...
def _relocate(tensor: Any, memory_config: Any, dtype: Any = None) -> Any:
    ...
    Ported from ``llama33_70b_galaxy.model._relocate``, which was rewritten
    after the three-argument form aborted Llama's first decode on silicon. The
    defect is not model-specific and this copy carried it unchanged.
    ...
    * ``to_memory_config(t, memcfg)`` between two shard specs differing in both
    ...
    if source_memcfg.is_sharded():
        if not target_memcfg.is_sharded():
            ...
            #     TT_FATAL ... Kernel group cores do not match sub device cores
            #                  for programmable core type TENSIX
            # The bug is latent for every interleaved target that
...
class Qwen3_32BGalaxyTransformer2D(LightweightModule):
    ...
    def project_prefill_logits(
        ...
        *,
        ...
        ``release_hidden`` is the caller's statement about ownership. The
        default releases ``hidden`` as soon as the norm has consumed it, which is
        what every caller before tracing wanted: it is that caller's own
        transient and freeing it early lowers the peak. The **runtime** contract
        is the opposite — `finish_regular_prefill` hands over a hidden state the
        runtime owns, and under tracing that tensor is the captured graph's own
        persistent output buffer (`prefill/trace.py`,
        ``PrefillReplayOwnership.trace_owned_hidden_output``). Freeing it there
        does not fail on the replay that freed it; it fails on the **next** one,
        as a segmentation fault inside the first op to read the reused address.
        Observed in ``rms_norm_pre_all_gather``, on the second of thirty-two
        prefills.
        """
    ...
    def close(self) -> None:
        ...
        if self.config.owns_shared_resources:
            ...
            # `prefetcher.cleanup()`. Before this release existed nothing
            # returned them - `close()` returned with every `wqkv`, `wo`, `w1`,
            # `w2` and `w3` still on the device for as long as any caller held
            # the model, which is 19 931 264 B per DRAM bank at a four-layer
            # subset and 398 617 984 B - 37 % of the bank - at 80 layers.
            #
