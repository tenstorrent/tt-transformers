...
"""Hugging Face generation and loading for the TTTv2 Llama-3.1-8B path."""
...
def resolve_hf_model_id(hf_model: str | None = None) -> str:
    return hf_model or environment_value("HF_MODEL") or DEFAULT_HF_MODEL
...
def _model_cache_path(hf_model: str, mesh_device, *, hf_revision: str | None = None, dtype=None) -> Path:
    ...
    if resolution.fallback_from is not None:
        logger.warning(
            f"Configured TT cache is not writable at {resolution.fallback_from}; "
            f"using job-local tensor cache {resolution.path}"
        )
...
def _max_prefill_chunk_size(mesh_device) -> int:
    override = environment_int("MAX_PREFILL_CHUNK_SIZE")
...
def _disable_batched_prefill(mesh_device) -> bool:
    ...
    return get_device_name(mesh_device) in {"P150", "P300", "P150x4", "P150x8"} or bool(
        environment_flag("DISABLE_BATCHED_PREFILL")
    )
...
def _load_model(
    ...
    *,
    ...
    from tt_transformers.models.llama3_8b.model import Llama3Transformer1D, build_llama3_transformer_1d_config
    ...
    model_cache_path = _model_cache_path(hf_model, mesh_device, hf_revision=hf_revision, dtype=dtype)
...
def from_pretrained(
    ...
    *,
    ...
    paged_attention_config=None,
    converted_state_dict: dict[str, torch.Tensor] | None = None,
    executor_config: Llama3ExecutorConfig | None = None,
):
    ...
    the existing ``Llama3ExecutorConfig`` for explicit trace/sampling policy.
    """
    if executor_config is not None and not isinstance(executor_config, Llama3ExecutorConfig):
        raise TypeError("executor_config must be a Llama3ExecutorConfig")
    ...
    try:
        ...
        llm.executor = build_llama3_executor(llm, executor_config)
        try:
