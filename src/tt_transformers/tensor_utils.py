...
def program_config_to_dict(program_config):
    ...
    attributes = {}
    for name in dir(program_config):
        if name.startswith("_"):
            continue
        try:
            value = getattr(program_config, name)
        except (AttributeError, RuntimeError, TypeError):
            continue
        if callable(value):
            continue
        attributes[name] = _program_config_value_to_json(value)
    if attributes:
        return {"type": type(program_config).__name__, **attributes}

    try:
        representation = repr(program_config)
    except (RuntimeError, TypeError):
        representation = f"<{type(program_config).__name__}>"
    return {"type": type(program_config).__name__, "repr": representation}


def _program_config_value_to_json(value):
    if value is None or isinstance(value, (bool, int, float, str)):
        return value
    if isinstance(value, dict):
        return {str(key): _program_config_value_to_json(item) for key, item in value.items()}
    if isinstance(value, (list, tuple)):
        return [_program_config_value_to_json(item) for item in value]
