#!/usr/bin/env python3
"""Verify the pinned canonical-sampling foundation extraction."""
...
def selected_rows() -> list[InventoryRow]:
    ...
    if set(rows) != SOURCES:
        raise ValueError(f"missing canonical-sampling inventory rows: {SOURCES - set(rows)}")
    result = []
...
def expected_content(row: InventoryRow) -> bytes:
    ...
    text = bytes(
        git_output("cat-file", "blob", f"{SOURCE_REVISION}:{row.source_path}", binary=True)
    ).decode("utf-8")
...
def main() -> int:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="rewrite canonical sampling destinations from the pinned snapshot",
    )
    ...
    if tensor_blob != PHASE4_TENSOR_UTILS_BLOB:
        failures.append(
            f"tensor_utils source blob mismatch: expected {PHASE4_TENSOR_UTILS_BLOB}, got {tensor_blob}"
        )
    if not PHASE4_TENSOR_UTILS_DESTINATION.exists():
        ...
        if tensor_hash != PHASE4_TENSOR_UTILS_SHA256:
            failures.append(
                f"Phase 4 tensor_utils drift: expected {PHASE4_TENSOR_UTILS_SHA256}, got {tensor_hash}"
            )
    if failures:
        print("\n".join(failures))
        return 1
    print("verified 3 pinned canonical-sampling files plus Phase 4 tensor_utils normalization")
    return 0
