...
import torch
import ttnn
from examples.common.auto_compose import extract_tensor_topology_info, get_device_from_tensor
...
def from_torch_dist_as(
    from_tensor_pt: torch.Tensor, as_tensor_tt: ttnn.Tensor, device: ttnn.MeshDevice | None = None
) -> ttnn.Tensor:
    """
    ...
    # Pattern 1: Using mesh_mapper without device (tensor stays in host memory) tt-metal docs, Programming_Mesh_of_Devices_with_TT-NN.md
    # Then transfer to device separately: same tt-metal doc
    # Pattern 2: Using both mesh_mapper and device together (direct to device) tt-metal llms.md
