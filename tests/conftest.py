# --- Pinned source segment: tests/conftest.py @ f8f547a30a2b4f920c745638f91647b8963e6f74 ---
...
def pytest_addoption(parser):
    parser.addoption(
        "--tt-arch",
        choices=[*ALL_ARCHS],
        default=os.environ.get("TT_ARCH", "wormhole_b0"),
...
def pytest_collection_modifyitems(config, items):
    ...
    # MESH_DEVICE is an explicit user/scheduler choice, so it is safe to add
    # architecture/SKU marks without probing hardware. Blank/unknown values add
    # nothing and therefore never fabricate topology coverage.
    from tests.support.marker_policy import selected_topology_marks

    topology_marks = selected_topology_marks(pytest)
    for item in items:
        if item.get_closest_marker("device"):
            for mark in topology_marks:
...
def ttnn_mesh_device(request):
    ...
    if parent_shape == (1, 1):
        ...
        if fabric_config is None:
            ...
            fabric_config = _default_fabric_config(req_shape)
        # set all other input arguments to default values by top-level tests/conftest.py
        ttnn.set_fabric_config(
            fabric_config, ttnn.FabricReliabilityMode.STRICT_INIT, None, ttnn.FabricTensixConfig.DISABLED
        )

    # config dispatch core to default values by tests/conftest.py
