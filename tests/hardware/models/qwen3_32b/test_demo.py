...
import pytest
from examples.qwen3_32b import benchmark as example
...
def test_qwen3_32b(test_config, mesh_device, optimizations):
    try:
        example.run_qwen3_32b(test_config=test_config, mesh_device=mesh_device, optimizations=optimizations)
    except example.UnsupportedConfiguration as error:
...
def test_qwen3_32b_p150x4_seeded_cross_cardinality(mesh_device):
    try:
        example.run_qwen3_32b_p150x4_seeded_cross_cardinality(mesh_device=mesh_device)
    except example.UnsupportedConfiguration as error:
        pytest.skip(str(error))
