...
import pytest
from examples.llama3_8b import benchmark as example
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "batch-32-ci",
        pytest.param("eval-32-repeat-3", id="eval-32-repeat_batch-3-prefetcher-off-perf-report-off"),
        pytest.param("eval-32-repeat-1", id="eval-32-repeat_batch-1-prefetcher-off-perf-report-on"),
        "ci-b1-DP-2",
        "ci-b1-DP-4",
        "ci-b1-DP-8",
        "ci-b1-DP-16",
...
def test_llama3_8b_bh_seeded_cross_cardinality(ttnn_mesh_device, optimizations):
    try:
        example.run_llama3_8b_bh_seeded_cross_cardinality(
            ttnn_mesh_device=ttnn_mesh_device, optimizations=optimizations
        )
    except example.UnsupportedConfiguration as error:
        pytest.skip(str(error))
