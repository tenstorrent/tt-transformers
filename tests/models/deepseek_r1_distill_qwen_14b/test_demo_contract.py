...
_DEMO_PATH = "examples/deepseek_r1_distill_qwen_14b/demo.py"
_HARDWARE_DEMO_PATH = "tests/hardware/models/deepseek_r1_distill_qwen_14b/test_demo.py"
_DEMO_SOURCE = "\n".join(
...
def test_eval_prefill_signature_multiset_is_rotation_invariant_and_not_static_warmup_shaped():
    ...
    # Standalone prefill planning does not split an oversized active batch into
    # multiple waves: the model cap is a fallback boundary.  The dedicated
    # prefill contract covers that policy; this demo check retains its original
    # rotation-invariance intent against the resulting sequential requests.
...
def test_main_demo_does_not_synchronize_parent_mesh_after_prebuild_skip():
    function = next(
        node
        for node in _DEMO_TREE.body
        if isinstance(node, ast.FunctionDef) and node.name == "run_deepseek_r1_qwen_14b"
    )
