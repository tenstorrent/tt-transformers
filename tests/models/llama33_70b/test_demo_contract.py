...
def test_eval_perf_report_is_dispatched_for_both_profiles_and_resolves_target():
    source = ast.unparse(_function("run_llama33_70b"))
    ...
    preflight = _calls("run_llama33_70b", "_preflight_perf_target")[0]
    create = _calls("run_llama33_70b", "create_model")[0]
...
def test_local_perf_nodes_observe_without_floor_and_enforce_complete_floor():
    ...
    assert "if not not failures" in perf_source
    assert "raise AssertionError" in perf_source
