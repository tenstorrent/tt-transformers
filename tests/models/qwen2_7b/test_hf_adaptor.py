...
from tt_transformers.models.qwen2_7b import hf_generator as hf_adaptor
from tt_transformers.models.qwen2_7b import model as qwen_model
from tt_transformers.models.qwen2_7b import weight_utils
from tt_transformers.models.qwen2_7b.hf_generator import Qwen2ForCausalLM as Qwen2Product
from tt_transformers.models.qwen2_7b.hf_generator import Qwen2RuntimeConfig, _trace_seq_lens, convert_hf_model_weights


@pytest.mark.host
@pytest.mark.model
def test_runtime_config_preserves_tp2_trace_and_batched_prefill_policy():
    runtime = Qwen2RuntimeConfig(
...
def test_provider_rejects_non_tp2_before_loading_hf(expect_error):
    ...
    with expect_error(ValueError, "TP2/N300 only"):
...
def test_product_binds_runtime_config_and_stop_tokens():
    ...
    product = Qwen2Product(model=model, tokenizer=tokenizer, runtime_config=runtime)
...
def test_tokenizer_adds_eos_and_im_start_and_threads_revision(monkeypatch):
    ...
    assert hf_adaptor.load_tokenizer("Qwen/Qwen2-7B-Instruct", "revision") is tokenizer
...
def test_config_builder_is_owned_by_model_module():
    assert hf_adaptor.build_qwen2_7b_transformer_config is qwen_model.build_qwen2_7b_transformer_config
    assert qwen_model.build_qwen2_7b_transformer_config.__module__ == qwen_model.__name__
...
def test_decoder_layer_prefill_calls_chunk_capable_attention_entrypoint(monkeypatch):
    ...
    layer = qwen_model.Qwen2_7BDecoderLayer(
