...
def test_demo_imports_every_called_shared_run_helper():
    imported = {
        ...
        if isinstance(node, ast.ImportFrom) and node.module == "examples.common.run_helpers"
...
def test_cross_cardinality_experiment_is_one_canonical_exact_token_node():
    function = _function("run_qwen3_32b_p150x4_seeded_cross_cardinality")
    ...
    assert len(_calls("run_qwen3_32b_p150x4_seeded_cross_cardinality", "make_executor")) == 2
    make_calls = _calls("run_qwen3_32b_p150x4_seeded_cross_cardinality", "make_executor")
    expected_policies = {
        ast.literal_eval(
...
def test_cross_cardinality_environment_and_checked_in_policy_fail_closed():
    ...
    create_call = _calls("run_qwen3_32b_p150x4_seeded_cross_cardinality", "create_model")[0]
    assert not any(keyword.arg == "disable_batched_prefill" for keyword in create_call.keywords)
    canonical_calls = _calls("run_qwen3_32b", "create_model")
...
def test_eval_perf_report_reuses_three_repeat_geometry_and_first_repeat_profiler():
    ...
    assert "eval-32-perf-report" in ast.unparse(_function("run_qwen3_32b"))
...
def test_main_demo_resolves_eval_floor_by_optimization_profile_and_local_perf_only_gates_complete_floor():
    main_source = ast.unparse(_function("run_qwen3_32b"))
