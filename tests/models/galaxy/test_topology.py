...
def test_wormhole_descriptor_reproduces_the_qualified_tables_in_order():
    ...
    assert topology.prefill_sdpa_k_chunk == 512
...
@pytest.mark.host
@pytest.mark.model
def test_the_long_sequence_sdpa_k_chunk_is_the_only_chunk_size_that_splits():
    """`sdpa_program_config` resolves the reference's one architecture split.

    The reference spells it
    ``k_chunk_size=(256 if self.is_blackhole else 512)``, guarded by
    ``seqlen >= 2048 and chunk_start_idx == 0``, and leaves ``q_chunk_size`` at
    256 on both. So the pair is *asymmetric on Blackhole* -- 256/256 against
    Wormhole's 256/512 -- and only the ``k`` half is on the descriptor. Asserted
    through the recipe rather than off the field, because the field is only worth
    having if the program config actually carries it.

    The two regimes that do **not** split are asserted alongside, since a future
    parameterization of either would be a behaviour change rather than a
    refactor: sub-2048 prefill is 64/64 and decode is 0/0 on both architectures.
    """

    wormhole = resolve_galaxy_chip_topology(_mesh())
    blackhole = resolve_galaxy_chip_topology(_mesh(arch=ttnn.device.Arch.BLACKHOLE, grid=(12, 10), dram_width=8))

    assert wormhole.prefill_sdpa_k_chunk == 512
    assert blackhole.prefill_sdpa_k_chunk == 256

    for topology, expected_k in ((wormhole, 512), (blackhole, 256)):
        long_sequence = recipes.sdpa_program_config(2048, decode=False, topology=topology)
        assert long_sequence.q_chunk_size == 256
        assert long_sequence.k_chunk_size == expected_k

        short_sequence = recipes.sdpa_program_config(128, decode=False, topology=topology)
        assert (short_sequence.q_chunk_size, short_sequence.k_chunk_size) == (64, 64)

        decode = recipes.sdpa_program_config(2048, decode=True, topology=topology)
        assert (decode.q_chunk_size, decode.k_chunk_size) == (0, 0)
...
@pytest.mark.host
@pytest.mark.model
def test_reduce_scatter_worker_count_survives_the_envelope_origin_shift():
    """The worker count `ttnn.reduce_scatter` can actually place, per envelope.

    `reduce_scatter` is the one collective that offsets its worker selection,
    and it offsets by the worker sub-device's own origin -- which the selection
    walk already starts from, so the origin is counted twice. Every `all_gather`
    factory passes `(0, 0)` instead, which is why the two behaved differently on
    the same Blackhole envelope in the same run: `all_gather` passed and
    `reduce_scatter` aborted on placement.

    The three cases below are the three regimes, and only one of them is about
    Blackhole:

    * **origin at the grid origin** -- Wormhole prefill's single full-grid
      worker. Nothing shifts, so the whole envelope is placeable, the answer is
      `None`, and the op's own data-sized default stands. No Wormhole value
      moves.
    * **origin one column in** -- Blackhole's prefetcher-free envelope, which the
      mode plan gives to prefill as well as decode. The shift costs the last
      column, so 9 of 100 cores are placeable and only the smallest selection
      fits.
    * **origin inside a split envelope** -- Wormhole decode, where the shift
      walks straight into the prefetch sender column. Nothing fits, and
      `ttnn.reduce_scatter` is independently documented as unusable under that
      partition. Reaching that conclusion from the geometry rather than from the
      recorded error is the point of the check.
    """

    wormhole_prefill = ((0, 0, 6, 9),)
    assert ccl_offset_placeable_worker_cores(wormhole_prefill) == 70
    assert reduce_scatter_workers_per_link(wormhole_prefill, 4) is None

    blackhole = resolve_galaxy_chip_topology(
        _mesh(arch=ttnn.device.Arch.BLACKHOLE, grid=(12, 10), dram_width=8)
    ).worker_core_ranges
    assert blackhole == ((1, 0, 10, 9),)
    # Column 10 shifts to column 11, which is the dispatch column and outside the
    # envelope, so the walk stops one short of the first full row.
    assert ccl_offset_placeable_worker_cores(blackhole) == 9
    assert reduce_scatter_workers_per_link(blackhole, 2) == 1

    # Per direction, and there are two, plus a mux core per direction above one
    # worker: 4, 12, 20, 36 on two links -- not 4, 8, 16, 32. The refused
    # selection was 12; the measured error named 12 selected cores.
    assert [reduce_scatter_worker_cores(workers, 2) for workers in (1, 2, 4, 8)] == [4, 12, 20, 36]
    assert reduce_scatter_worker_cores(1, 4) == 8

    wormhole_decode = WORMHOLE_GALAXY_TOPOLOGY.worker_core_ranges
    assert ccl_offset_placeable_worker_cores(wormhole_decode) == 2
    with pytest.raises(ValueError, match="no reduce_scatter worker count fits"):
        reduce_scatter_workers_per_link(wormhole_decode, 4)
