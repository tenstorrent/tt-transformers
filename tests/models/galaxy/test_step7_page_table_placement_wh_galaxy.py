...
"""The one host assumption under the decode page-table defect, settled on silicon.

The defect - *a prefill-shaped page table fed to decode is accepted, not
rejected* - was derived entirely on the host, from a model of one non-obvious
``ttnn`` fact: **a distributed tensor's ``.shape`` is the shard shape, not the
global one**, as read out of ``ttnn/core/distributed/distributed_tensor.cpp``.

One line settles it. If the device-local shape is ``(32, 64)`` instead, the
defect is worse than described: the "device-local rows" branch would be
unreachable for a correctly-mapped table, so decode's page table would have no
effective validation at all.

This file is that line, as a test. It needs a mesh and no model: three
``ttnn.from_torch`` calls, no weights, no sub-devices, about twenty seconds.
...
def test_a_column_sharded_page_tables_device_local_view_is_one_columns_users(mesh_device: ttnn.MeshDevice):
    ...
    try:
        ...
        assert shape == (_USERS_PER_COLUMN, _BLOCKS), (
            ...
            "If this is (32, 64) then the defect is worse than reported: the decode validator's "
...
def test_a_replicated_page_tables_device_local_view_is_the_whole_physical_batch(mesh_device: ttnn.MeshDevice):
    """And 32 is a multiple of 8, which is the whole of the defect."""
    ...
    try:
        ...
        assert shape[0] % _USERS_PER_COLUMN == 0, "the premise of the defect no longer holds"
    finally:
        ttnn.deallocate(table)
