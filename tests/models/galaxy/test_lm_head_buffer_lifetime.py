...
comment at `lm_head_all_reduce` for the silicon behind this. With the buffer
hoisted, the traced decode is fourteen of fourteen exact.
...
def test_only_the_decode_lm_head_all_reduce_provisions_an_l1_intermediate(model, _host_mesh_mappers):
    ...
    room for exactly one of them, as measured on silicon. A second key acquiring
    an L1 intermediate would be silently unaffordable, and the decode graph would
    stop placing rather than say so.
    """
