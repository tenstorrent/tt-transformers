...
from tt_transformers.modules.rmsnorm.rmsnorm_2d import RMSNorm2DConfig, RMSNorm2DGeometry, _resolve_2d_config
...
def test_geometry_reports_galaxy_aligned_vocabularies():
    # Ring-exact, not minimal: see `galaxy_padded_vocab_size`. A width
...
def test_the_lm_head_reduction_leaves_the_collective_worker_cores():
    ...
    Measured on `(8, 4)`: Qwen3-32B's 19200-wide local logits are 600
    ...
    for cores, local in ((llama_cores, llama_local), (qwen_cores, qwen_local)):
        ...
        # The ring-exactness invariant, unchanged: a tensor handed to
        # `all_reduce_async` must have `logical_width == cores * shard_width` exactly.
...
def test_rope_batch_grid_lies_inside_the_worker_sub_device(use_qk_fused):
    ...
    This is the same failure shape as earlier sub-device defects - a grid named
