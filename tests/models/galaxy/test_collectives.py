...
from tt_transformers.models.galaxy.collectives import GalaxyAttentionCollectives, GalaxyColumnUserSelector
from tt_transformers.models.galaxy.recipes import GALAXY_MESH_SHAPE, GalaxyDenseGeometry
...
def collectives(monkeypatch):
    ...
    resources = SimpleNamespace(
        context=lambda mode: SimpleNamespace(
            ...
            # `None` is the Wormhole answer, and the one the real context gives
            # for the full-grid prefill partition this branch runs under: nothing
            # shifts, so `reduce_scatter`'s own worker count is placeable. The
            # method has to exist even so, because the call site is unconditional
            # -- that is what keeps a shifted Blackhole partition from silently
            # getting the default and aborting on placement.
