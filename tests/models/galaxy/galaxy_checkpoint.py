...
The one-block qualification needs the real checkpoint's layer 0 and nothing
else. The obvious way to get it -
...
process, and the house rule is three fresh processes per claim.
...
Two properties are preserved deliberately, because an earlier post-mortem
turned on both:
...
turn that into ``pytest.skip``. Inventing weights instead is not an option: the
point of the exercise is the checkpoint's real tensors.
"""
