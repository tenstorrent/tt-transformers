...
The first hardware session lost several device runs to the same class of defect:
a decode-mode program placed on cores the loaded sub-device manager does not
own, or on dispatch cores. Both are properties of *this* mesh and this
...
Run this file first when a decode program aborts with either of:
...
import torch
import ttnn
from tests.models.galaxy.galaxy_hardware import GALAXY_DEVICE_PARAMS, GALAXY_MESH_SHAPE
