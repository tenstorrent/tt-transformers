...
def test_the_all_reduce_scratch_width_is_its_own_shard_specs_width(model):
    """A width-sharded tensor's width *is* cores x shard width, so derive it.

    This buffer used to be shaped `(*mesh, TILE, 50 * 1024)` against a shard spec
    of `(TILE, 1024)` over `worker_cores()`. Those agree only where the worker
    envelope happens to hold 50 cores, which is Wormhole -- so the literal was
    that architecture's core count in disguise, and a B8-class pinned constant
    that B8's inventory never listed. Blackhole's envelope is 100 cores, where
    the same literal describes half the tensor its own spec shards.

    Asserted against the spec rather than against a number, because the point is
    that the two cannot disagree: `test_recipes.py` already pins the grid to
    `worker_cores()`, and this pins the width to that grid.
    """

    _, config = _resources(model)
    plan = next(p for p in config.decode.collectives if p.key.operation == "all_reduce" and p.key.cluster_axis == 0)
    spec = plan.persistent_output_specs[0]
    shard = spec.memory_config.shard_spec

    assert spec.shape[-1] == shard.grid.num_cores() * shard.shape[1], (
        f"the scratch is {spec.shape[-1]} wide but its shard spec describes "
        f"{shard.grid.num_cores()} cores x {shard.shape[1]} = "
        f"{shard.grid.num_cores() * shard.shape[1]}"
    )
    # Wormhole's qualified value, unchanged: 50 worker cores x 1024.
    assert spec.shape[-1] == 50 * 1024
...
def test_decode_plan_covers_exactly_the_qualified_collectives(model):
    ...
    assert keys == {
        ...
        # the collective was added to the decode plan (it needs its own keyed
        ...
        # logits - 16128 for Llama, 19200 for Qwen, both ring-exact.
...
def test_mode_plans_fail_closed_on_a_semaphore_set_narrower_than_the_workers():
    """A sender on an uncovered core hangs; promoted from a test helper into the plan.
