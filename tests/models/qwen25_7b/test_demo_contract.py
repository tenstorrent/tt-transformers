...
_DEMO_PATH = "examples/qwen25_7b/benchmark.py"
_HARDWARE_DEMO_PATH = "tests/hardware/models/qwen25_7b/test_demo.py"
_DEMO_SOURCE = "\n".join(
...
def test_main_demo_does_not_synchronize_parent_mesh_after_prebuild_skip():
    function = next(
        node for node in _DEMO_TREE.body if isinstance(node, ast.FunctionDef) and node.name == "run_qwen25_7b"
    )
