...
def _runtime_config():
    return Qwen25Coder32BRuntimeConfig(
...
def test_pinned_revision_is_provider_and_generator_default():
    expected = "381fc969f78efac66bc87ff7ddeadb7e73c218a7"
    assert hf_adaptor.DEFAULT_HF_REVISION == expected
    assert generator.Qwen25Coder32BGeneratorConfig.__dataclass_fields__["hf_revision"].default == expected
...
def test_product_binds_runtime_config_and_qwen_stop_tokens():
    ...
    tokenizer = SimpleNamespace(stop_tokens=[151403, 151404])
    product = Qwen25Coder32BForCausalLM(model=model, tokenizer=tokenizer, runtime_config=_runtime_config())
    assert model.model_args is product.runtime_config
    assert product.generation_config.stop_token_ids == (151403, 151404)
...
def test_qwen_stop_tokens_include_turn_terminators():
    token_map = {"<|im_end|>": 151405, "<|im_start|>": 151404}
    tokenizer = SimpleNamespace(
        eos_token_id=151403,
        convert_tokens_to_ids=lambda token: token_map.get(token, -1),
    )
    assert hf_adaptor._qwen_stop_token_ids(tokenizer) == (151403, 151405, 151404)
...
def test_convert_hf_model_weights_covers_real_nonempty_qwen_layer():
    ...
    assert weights.layers[0].wqkv.shape == (1, 1, 320, 448)
    assert weights.layers[0].wqkv_bias.shape == (448,)
    assert weights.lm_head.shape == (128, 320)
