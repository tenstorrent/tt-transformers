...
from tt_transformers.models.galaxy.recipes import resolve_galaxy_decode_placements
from tt_transformers.models.llama33_70b_galaxy import hf_adaptor, weight_utils
from tt_transformers.models.llama33_70b_galaxy import model as galaxy_model
...
from tt_transformers.modules.rmsnorm.rmsnorm_2d import RMSNorm2DResidualPolicy, _resolve_2d_config
...
@pytest.mark.host
@pytest.mark.model
def test_checkpoint_contract_rejects_an_out_of_range_layer_subset(expect_error):
    with expect_error(ValueError, r"n_layers must be in \[1, 80\]"):
...
def test_parameters_resolve_the_galaxy_geometry():
    ...
    # the decode LM head's column all-reduce forever. The padding is masked to
    # -inf by LMHead2D, and it is what makes that mask load-bearing for
...
@pytest.mark.host
@pytest.mark.model
def test_fused_qkv_rejects_widths_that_cannot_shard_over_rows(expect_error):
    with expect_error(ValueError, "must shard over 8 mesh rows"):
...
@pytest.mark.host
@pytest.mark.model
def test_lm_head_weight_is_transposed_and_zero_padded(expect_error):
    ...
    with expect_error(ValueError, "LM-head weight must have shape"):
...
def test_prefetch_registration_is_ordered_per_layer():
    ...
    # 0.737 and decode MLP 0.096 with all five registered.
...
def test_device_sampling_names_the_decode_worker_sub_device():
    ...
    top-k, so every sampled token after the first is stale. Nothing on device
    catches it either, because the accidental serializers on a cache miss
    ...
    config = galaxy_model.build_llama33_70b_galaxy_transformer_2d_config(
        mesh_device=mesh,
        geometry=params.geometry(),
        precision=LLAMA33_70B_GALAXY_ACCURACY,
        lazy_weights=lazy,
...
def test_distributed_norm_resolves_its_statistics_onto_the_decode_input_origin(monkeypatch):
    ...
    unrelated L1, which `RMSNorm2D` now rejects outright via
    `_require_fused_stats_placement`. This model must therefore not name a
