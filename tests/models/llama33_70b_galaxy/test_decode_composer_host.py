...
"""Focused host test for the one common-runtime change `c-trace` needed.

`DecodeRuntime` composed multi-device decode output itself, with a
column-concatenating composition that is correct for a 1D model and is finding
D-B23 on Galaxy — and, more importantly here, it composed it *behind* the model
...
def test_the_probe_is_a_capability_check_and_not_a_model_branch():
    ...
    the 1D caller that owns this runtime, and has since Milestone A; a docstring
