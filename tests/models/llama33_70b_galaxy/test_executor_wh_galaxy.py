...
"""Device coverage for `Llama33_70BGalaxyExecutor` on WH Galaxy `(8, 4)`.
...
**What is no longer here.** Coverage items 1 and 3 used to compare the
executor against a reference recorded through `GalaxyDirectRunner`. The runner
is deliberately absent (2026-09-02
...
def _prompt(length: int) -> list[int]:
    ...
    # The reference text holds 1024 tokens and this case needs 2048. Repeating
...
def test_executor_prefix_cached_prefill(mesh_device: ttnn.MeshDevice) -> None:
    ...
    Previously blocked for Llama by the L1 address clash, which has since been
    fixed.
    ...
    * the cached prefix is one **chunk alignment** (128), not one page-table
      ...
      It is the same reason the warmup plan's prefix moved;
...
def test_executor_warmup_and_program_identity(mesh_device: ttnn.MeshDevice, order: str) -> None:
    ...
    order, so both orders are measured. They are not equivalent on this mesh: one
    order compiles one decode program and then hits the Llama L1 address clash on
    the very next prefill (`program 922`, L1 buffer at 543488, core range
    `[0-0 - 0-3]`, from `Embedding2D._forward`). That is an open defect, reached
    here by a much cheaper route than a 141-second probe, and the parametrization
    is how this file reports it rather than avoids it.
    """
...
def test_executor_teacher_forced_accuracy(mesh_device: ttnn.MeshDevice) -> None:
    ...
    The same convention as the `GalaxyDirectRunner` measurement of
