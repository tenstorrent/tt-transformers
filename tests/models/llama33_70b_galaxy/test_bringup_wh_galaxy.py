...
"""Hardware bring-up for the Galaxy Llama-3.3-70B model.
...
This file deliberately asserts *nothing about numerics*. Construction,
prefetcher sealing, CCL resource resolution and clean teardown all have to be
proven before a single PCC number is looked at, because each of the four things
below fails in a different and differently-diagnosable way, and a combined test
...
``test_one_decode_step_executes``
    ...
    (attention had only ever been qualified with an *identity* rotary), and the
    ring/``gather_in0`` decode QKV matmul running inside the prefetcher's worker
    subdevice partition. The latter was terminal against the old straddling
    ``(7, 1)`` grid. Either failure is a
...
``test_two_models_in_one_process``
    ``Prefetcher2D.cleanup()`` cannot free the global
...
from tests.models.galaxy.galaxy_checkpoint import (
...
def _build(mesh_device, weights, params):
    return build_llama33_70b_galaxy_model(
        ...
        # Paged KV is qualified separately; this file qualifies contiguous.
...
def test_two_models_in_one_process(mesh_device, layer0_weights_and_params):
    """Probe the prefetcher's L1 retention limitation directly.
