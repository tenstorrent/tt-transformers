...
"""Tracing coverage for `Llama33_70BGalaxyExecutor` on WH Galaxy `(8, 4)`.
...
def test_eager_and_traced_decode_logits_agree_with_device_sampling_built(mesh_device: ttnn.MeshDevice) -> None:
    ...
    **three** capture bodies and its PCC is a claim about that configuration
    ...
    try:
        ...
        # Reported, not asserted: the threshold this node gates on is the PCC
        # threshold, and exactness is not part of it. It is printed because the
...
def test_paged_kv_late_capacity_under_tracing(mesh_device: ttnn.MeshDevice, expect_error) -> None:
    ...
    is an open `Attention2D` paged-KV contract defect with a page-table
    width bound by the pool size, already reported and independently reproduced
    on Qwen. Nothing here relaxes it.
    """
...
def test_repeated_startup_serving_and_cleanup_with_tracing(mesh_device: ttnn.MeshDevice) -> None:
    ...
    the cleanup fix than the eager cycle was.
    """
