...
"""Hardware qualification for the Galaxy Llama-3.3-70B model.

One decoder layer, prefill 128 and one decode step, compared against the
Hugging Face reference for the *same* single layer.

**The assertions here are unvalidated.** This file was written statically,
against the reference rather than against a Galaxy mesh, so that the first
hardware session starts from a concrete test rather than an empty file. Treat
every assertion as a hypothesis until it runs. The assumptions it encodes, in
the order they are most likely to be wrong:
...
1. **Contiguous KV, not paged.** The model is built with
   ...
   column; the qualified attention test therefore also ran
...
Run it as::
    ...
    pytest tests/models/llama33_70b_galaxy/test_model_wh_galaxy.py -v
...
def _reference_logits_and_cache(hf: Any, tokens: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    which is the independent reference this file wants for the cache
    contents: ``K`` is post-RoPE and ``V`` is the raw value projection, exactly
    what the device writes. Deriving it from a hand-written re-implementation
    hides errors on both sides.
...
def test_llama33_70b_galaxy_decode_bisection(mesh_device: ttnn.MeshDevice):
    ...
    try:
        ...
        try:
            ...
            from tt_transformers.models.llama33_70b_galaxy.model import DecodeMetadata
