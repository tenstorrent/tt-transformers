...
"""Hardware qualification for the Galaxy Qwen3-32B model.
...
of ``tests/models/llama33_70b_galaxy/test_model_wh_galaxy.py``:
...
1. **The 64-head decoupled geometry.** ``n_heads * head_dim`` is 8192 while
   ...
   is ``attention_dim``-wide rather than ``dim``-wide. Earlier Qwen3-32B attention
   results were measured against a **40-head** fixture
...
2. **Per-head Q/K normalization.** ``Attention2D`` applies ``RMSNorm2D`` with
   ``HEAD_LOCAL`` geometry to the created Q and K heads. Head-local decode used to
   abort in op validation before producing any numerical result at all, so there
   is no prior Qwen Q/K norm number anywhere.
...
Run it as::
    ...
    pytest tests/models/qwen3_32b_galaxy/test_model_wh_galaxy.py -v
...
that skipped is a failure of the run, not a result**: point `HF_HOME` at a
cache that actually holds Qwen3-32B.
"""
...
import torch
import ttnn
from tests.models.galaxy.galaxy_checkpoint import CheckpointUnavailable, load_layer_subset_causal_lm
from tests.models.galaxy.galaxy_hardware import hf_config_or_skip
...
from tt_transformers.models.qwen3_32b_galaxy.weight_utils import reverse_permute_1d as qwen_reverse_permute_1d
...
def test_qwen3_32b_galaxy_geometry_is_decoupled_8x4_qwen3_32b(mesh_device: ttnn.MeshDevice):
    ...
    from tt_transformers.models.galaxy.recipes import dram_sharded_weight_memory_config
...
def test_qwen3_32b_galaxy_qk_norm_head_local_8x4_qwen3_32b_decode_and_prefill(mesh_device: ttnn.MeshDevice):
    ...
    A `HEAD_LOCAL` decode used to abort in op validation before producing any
    numerical result, so no prior Qwen Q/K norm number exists anywhere. It has to
    be validated *before* it is trusted inside the block, because a failure here
    and a failure in the decoupled attention geometry look identical from the
    block's logits.
    ...
    the decode manager is exactly the open question.
    ...
    #    keeps the norm's program inside the decode partition.
                ...
                try:
                    ...
                    if mode == "decode":
                        ...
                        assert returned == _cores(memcfg.shard_spec.grid), (
                            f"{name} came back on {sorted(returned)}, not {sorted(_cores(memcfg.shard_spec.grid))}"
                        )
...
def test_qwen3_32b_galaxy_one_layer_prefill_and_decode_8x4_qwen3_32b_b32_s128(mesh_device: ttnn.MeshDevice):
    """The block gate: prefill 128 and decode, logits and both caches."""
    ...
    try:
        ...
        try:
            ...
            # The cache contents, not just the block output: the threshold
...
def test_qwen3_32b_galaxy_decode_bisection_8x4_qwen3_32b_b32_s128(mesh_device: ttnn.MeshDevice):
    ...
    try:
        ...
        try:
            ...
            from tt_transformers.models.qwen3_32b_galaxy.model import DecodeMetadata
