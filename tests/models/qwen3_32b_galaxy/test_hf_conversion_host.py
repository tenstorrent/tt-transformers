...
"""Host qualification of the Qwen3-32B Galaxy adaptor.
...
So the layout conversion and the Qwen RoPE parameters have to be confirmed
*numerically* on host before anything reaches the mesh, because
...
* the **decoupled 64-head geometry** - ``n_heads * head_dim == 8192`` against
  ...
  ``wo`` projection really does reduce ``attention_dim`` to ``dim``. Earlier work
...
read out of ``tt_transformers.tensor_utils.get_rot_transformation_mat``, the same
...
import pytest
import torch
from tests.modules import _hf_reference
from tests.support.comparison import comp_pcc

from tt_transformers.models.qwen3_32b_galaxy import weight_utils
from tt_transformers.models.qwen3_32b_galaxy.model import QWEN3_32B_CHECKPOINT_CONTRACT, QWEN3_32B_GALAXY_HF_MODEL
...
# above the 0.99 model threshold on purpose: a layout error shows up as a
...
# =============================================================================
# The 64-head decoupled geometry: the primary layout risk
# =============================================================================
...
def test_real_qwen3_32b_geometry_is_decoupled_and_derives_the_contracted_widths():
    ...
    Earlier Qwen3-32B attention results were measured against a 40-head fixture
    chosen so that ``n_heads * head_dim == dim``. This asserts
    the real thing: ``attention_dim`` is 8192 against ``dim`` 5120, and the two
    are *not* interchangeable anywhere.
    """

    from tt_transformers.models.galaxy.recipes import GalaxyDenseGeometry
...
# =============================================================================
# Per-head Q/K normalization: the second layout risk
# =============================================================================
...
def test_head_local_rms_norm_in_meta_layout_reproduces_hf_qk_norm(small_qwen3):
    ...
    This is the number that did not exist anywhere before this file: head-local
    decode aborted in op validation *before producing any numerical result*, so
    there was no prior Qwen Q/K-norm value to compare against.
    """
