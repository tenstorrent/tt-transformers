...
"""Tracing coverage for `Qwen3_32BGalaxyExecutor` on WH Galaxy `(8, 4)`.
...
runner is frozen and scheduled for removal (decision of 2026-09-02), and — more
to the point — a traced-vs-runner comparison
...
Run one node id per process, as the house rules require::
    ...
    pytest tests/models/qwen3_32b_galaxy/test_trace_wh_galaxy.py \
...
#: (`models/model_trace_region_sizes.yaml`, key `qwen3-32b-galaxy`), reserved
...
#: cost three device runs to get here: the decode capture body ended in
#: a host barrier the Galaxy collectives perform in-graph, and the prefill
...
#: cores, which the prefill subdevice covers.
...
def _hf_revision() -> str | None:
    ...
    override = os.getenv("QWEN3_32B_HF_REVISION")
    if override:
        return override
...
def _open(handle: Any, **kwargs: Any) -> tuple[Qwen3_32BGalaxyExecutor, list[list[Any]]]:
    executor = Qwen3_32BGalaxyExecutor(handle.model, _runtime_config(handle), _executor_config(handle.model, **kwargs))
...
def test_eager_and_traced_decode_logits_agree_with_device_sampling_built(mesh_device: ttnn.MeshDevice) -> None:
    ...
    **three** capture bodies and its PCC is a claim about that configuration
    ...
    four, because the decode user gather's fabric-write destination was allocated and
    ...
    `1.0` in a log is not evidence of exactness.
    """
    ...
    try:
        ...
        # Reported, not asserted: exactness is not one of this node's gates. It is
        # printed because the residual is the thing the gate could not previously
        # see.
...
def test_paged_kv_late_capacity_under_tracing(mesh_device: ttnn.MeshDevice, expect_error) -> None:
    ...
    is an open `Attention2D` paged-KV contract defect with a page-table width
    bound by the pool size, already reported for Llama and independently
    reproduced on Qwen. Nothing here relaxes it.
    """
...
def test_mode_transition(mesh_device: ttnn.MeshDevice, transition: str, expect_error) -> None:
    ...
    persistent prefetcher for decode only.
    """
...
def test_repeated_startup_serving_and_cleanup_with_tracing(mesh_device: ttnn.MeshDevice) -> None:
    ...
    the resource-retention fix than the eager cycle was.
    """
...
# ---------------------------------------------------------------------------
# The capture-phase free hold — that it engages on real silicon
# ---------------------------------------------------------------------------
...
def test_capture_body_frees_are_held_for_the_capture_phase(mesh_device: ttnn.MeshDevice) -> None:
    """The hold engages inside the real capture bodies, and lets go after.
    ...
    try:
        ...
        # Nothing is retained past the capture phase: the hold is released in
        # full before the forty replays, which are bit-identical to eager on all
        # of them, so a resident hold is not what makes the trace correct.
        assert not hold._deferred, f"{len(hold._deferred)} tensors are still held after the capture phase"
        # The guard an earlier revision needed and did not have. A borrowed plan buffer
