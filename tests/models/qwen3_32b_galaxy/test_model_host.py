...
from tt_transformers.models.galaxy.plans import build_galaxy_decode_collectives
from tt_transformers.models.galaxy.recipes import resolve_galaxy_decode_placements, worker_cores
from tt_transformers.models.qwen3_32b_galaxy import hf_adaptor, weight_utils
from tt_transformers.models.qwen3_32b_galaxy import model as galaxy_model
...
from tt_transformers.modules.rmsnorm import rmsnorm_2d
...
def test_checkpoint_contract_fails_closed(overrides, message, expect_error):
...
def test_parameters_resolve_the_decoupled_head_geometry():
    ...
    # never sends - no abort, no traceback, a host hang. This assertion
    # was written against the earlier padding and has been stale since; the
...
@pytest.mark.host
@pytest.mark.model
def test_parameters_reject_an_impossible_layer_count(expect_error):
    with expect_error(ValueError, r"n_layers must be in \[1, 64\]"):
...
@pytest.mark.host
@pytest.mark.model
def test_head_local_vectors_use_the_same_interleave_as_the_projections(expect_error):
    ...
    with expect_error(ValueError, "width must be even"):
...
def _mesh():
    ...
    # The placement resolvers validate the topology descriptor against what the
    # device reports, so a fake that omits the compute grid is under-specified.
    # `MagicMock(spec=...)` answers the call with a Mock whose `.x`/`.y` are
    # Mocks too, which fails the descriptor's grid check rather than being
    # ignored -- so this is required, not decorative.
...
@pytest.mark.host
@pytest.mark.model
def test_model_assembly_requires_per_head_qk_norm_weights(expect_error):
    params = Qwen3_32BGalaxyModelParameters(n_layers=1)

    with expect_error(ValueError, "per-head Q and K normalization"):
        build_qwen3_32b_galaxy_model(object(), params=params, weights=_weights(1, qk_norm=False))


@pytest.mark.host
@pytest.mark.model
def test_model_assembly_rejects_a_layer_count_mismatch(expect_error):
    params = Qwen3_32BGalaxyModelParameters(n_layers=2)

    with expect_error(ValueError, "expected 2 layer weight sets, got 1"):
...
def test_prefetch_registration_is_ordered_per_layer():
    ...
    # confined worker rectangle (`dense_matmul_program_config`). Registering
    # `wqkv`/`wo` anyway put two unconsumed entries per layer into the buffer and
    # shifted every later consumer by one, so the MLP's `w1` read the entry meant
    # for `wqkv` - measured for Llama at decode MLP PCC 0.096 with every
    # configuration field correct.
...
def test_qk_norms_resolve_to_head_local_geometry():
    ...
    for weight in (layer.q_norm, layer.k_norm):
        ...
        # compute grid, which the loaded decode manager does not own -
...
@pytest.mark.host
@pytest.mark.model
def test_lazy_weights_reject_a_fused_qkv_bias(expect_error):
    mesh = _mesh()
    params = Qwen3_32BGalaxyModelParameters(n_layers=1)
    weights = _shaped_weights(params, layers=1)
    ...
    with expect_error(ValueError, "fused QKV bias"):
        build_qwen3_32b_galaxy_lazy_weights(
            mesh_device=mesh,
            geometry=params.geometry(),
            precision=QWEN3_32B_GALAXY_ACCURACY,
...
def test_batched_and_chunked_lengths_resolve_one_recipe_family_each():
    ...
    from tt_transformers.models.galaxy.recipes import resolve_galaxy_prefill_placements
...
@pytest.mark.host
@pytest.mark.model
def test_a_chunked_length_without_its_plain_recipe_fails_closed(expect_error):
    with expect_error(ValueError, "chunked prefill lengths must also be plain prefill lengths"):
...
def test_package_owns_its_graph_and_imports_no_model_named_implementation():
    for module in (galaxy_model, hf_adaptor, weight_utils):
        ...
        assert "tt_transformers.models.qwen3_32b." not in source
        assert "tt_transformers.models.llama33_70b" not in source
...
# ---------------------------------------------------------------------------
# Fused-norm statistics placement
# ---------------------------------------------------------------------------
...
def _norm_ccl(mesh):
    def context(mode):
        return SimpleNamespace(
            mesh_device=mesh,
            mode=mode,
            worker_sub_device_id="{mode}-worker",
...
def test_head_local_qk_norm_agrees_with_the_module_default_by_contract(monkeypatch):
    ...
    Qwen3 normalizes each `head_dim`-wide head independently. An earlier revision
    of the module resolved `HEAD_LOCAL` decode to a width-sharded L1 recipe; it
    now stays interleaved in DRAM like prefill. This model already passes
    `ttnn.DRAM_MEMORY_CONFIG` explicitly, so the two happen to agree - but nothing
    checked that, and a config that disagreed would have been rejected in op
    validation before producing a single number, which is exactly how that
    mismatch stayed hidden. Pin both sides.
    """
    ...
    # What the model asks for. Prefill agrees with the module's current default;
    ...
    # That is the unresolved half of the same problem, and agreeing with the
    ...
    # departing from": if the module's default ever moves, this diverges here on host.
...
def test_distributed_norm_resolves_its_statistics_onto_the_decode_input_origin(monkeypatch):
    ...
    unrelated L1, which `RMSNorm2D` now rejects outright
...
def test_embedding_decode_output_is_the_residual_placement_not_interleaved_l1():
    ...
    This was found on silicon for Llama and fixed there; this package carried
    the same `ttnn.L1_MEMORY_CONFIG` unchanged. Naming the
...
def test_wo_weight_placement_is_paired_with_attention_dim_not_dim():
    ...
    Earlier Qwen attention results were measured against a 40-head
...
@pytest.mark.host
@pytest.mark.model
def test_device_sampling_names_the_decode_worker_sub_device():
    """The sampler's gathers must be told the sub-device, like every other CCL.

    `ttnn.all_gather` takes no ``sub_core_grids``; without ``subdevice_id`` it
    places its workers on sub-device 0 - the prefetch senders - and fast dispatch,
    which orders programs per sub-device only, then never orders the gather
    against the ``topk`` feeding it. The symptom is not a crash but silence: with
    the program cache warm the gather consumes the *previous* decode step's
    top-k, so every sampled token after the first is stale. Nothing on
    device catches it either, because the accidental serializers on a cache miss
    make the first call - and every cold test - correct.
    """
    ...
    assert sampling.ccl_sub_device_id() is config.resources.context("decode").worker_sub_device_id
...
def test_the_fused_qk_rotary_is_the_default_everywhere_it_can_be_set():
    ...
    corrupt K into the cache: measured for Llama as a decode K with
...
def test_relocate_reaches_an_interleaved_target_in_one_hop():
    ...
    full compute grid. That is a real defect, and it is latent for every
