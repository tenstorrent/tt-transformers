...
"""Device coverage for `Qwen3_32BGalaxyExecutor` on WH Galaxy `(8, 4)`.

One test per coverage item:
...
2. paged KV — late capacity resolution, transactional bind/unbind, per-layer KV
   metadata;
3. prefix-cached and chunked prefill. **Both already passed for Qwen**, two fresh
   processes each, so a failure here is a regression and is reported as one;
...
5. three startup/serve/cleanup cycles in one process with no retained TT
   resources — the one-live-model form, which Qwen passed 3/3;
6. teacher-forced accuracy through the executor path, top-1 >= 89% / top-5 >= 97%;
7. **two models in one process** — the shape Qwen once *failed*, since fixed across
   four separate defects. Qwen is the only model of the two that can see the
...
**What is no longer here.** Coverage items 1 and 3 compared the executor against a
reference recorded through `GalaxyDirectRunner`. The runner is deliberately absent
(2026-09-02 operator decision) and nothing else in this package can produce those tensors,
...
def _prompt(length: int) -> list[int]:
    ...
    # The reference text holds 1024 tokens and the coverage asks for 2048. Repeating
...
def _context_for(length: int) -> int:
    ...
    instead of refusing (448 columns at ±inf).
...
def test_executor_prefix_cached_prefill(mesh_device: ttnn.MeshDevice) -> None:
    ...
    **This passed for Qwen** (two fresh processes), through `GalaxyDirectRunner`
    rather than through an executor. So a failure here is a regression — introduced
    either by shared-code changes or by this executor — and is reported as one, not
    absorbed.
    ...
    * the cached prefix is one **chunk alignment** (128), not one page-table
      ...
      That is also the reason the warmup plan's prefix moved;
    * the prefix has to be **in the cache** for the claim to mean anything, so
    ...
    refused on the host before any device work.
    """
...
def test_executor_warmup_and_program_identity(mesh_device: ttnn.MeshDevice, order: str) -> None:
    ...
    clash and it has since been fixed, so both orders are expected to pass
...
def test_executor_repeated_startup_and_cleanup(mesh_device: ttnn.MeshDevice) -> None:
    ...
    The one-live-model form, which Qwen passed 3/3. The two-models-in-one-process
    form is coverage item 8 below; both forms are required.
    """
...
def test_executor_teacher_forced_accuracy(mesh_device: ttnn.MeshDevice) -> None:
    """Coverage 7: teacher-forced accuracy through the executor path.

    The same convention as the `GalaxyDirectRunner` gate: prefill 512, then 511
    forced decode steps at batch 1. The thresholds asserted below are the gate;
    the executor must not lose accuracy against the runner on the same inputs.
    """
...
# ---------------------------------------------------------------------------
# 8. Two models in one process, through the executor
# ---------------------------------------------------------------------------
...
def test_executor_two_models_in_one_process(mesh_device: ttnn.MeshDevice) -> None:
    ...
    **This is the item Llama cannot provide.** Qwen once failed this shape, and the
    fix took four separate defects, each of which only became visible once the one
    before it was gone:

    * **L1 residue** — 923 776 of 1 393 472 B per L1 bank still allocated after
    ...
    * **global-CB allocation** — the second model's global circular buffer then
      failed to allocate on fragmentation;
    * **decode hang** — the second model's first decode then *hung*, because the ttnn
    ...
    * **DRAM residue** — Llama then failed on DRAM residue that Qwen never showed.

    The fix was verified through `GalaxyDirectRunner`, 3/3 at the full shape. This
    measures the same claim **through the executor**, which is the thing that did not
    exist then: the executor is the resource and cleanup root, so if it retains
    anything the first model owned, the second model's `activate("decode")` is where
    it shows.
    ...
    allocation. That is the shape that was qualified, and comparing the two at
