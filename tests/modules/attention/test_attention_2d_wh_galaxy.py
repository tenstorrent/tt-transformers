...
class _AttentionCollectives:
    ...
    def rotary(q: ttnn.Tensor, k: ttnn.Tensor, rot_mats: Any, **_: Any) -> tuple[ttnn.Tensor, ttnn.Tensor]:
        # RoPE has its own hardware qualification. Identity rotation keeps
...
def _reference_attention(spec: _ModelSpec) -> HfAttentionWrapper:
    ...
    identity on both sides: RoPE has its own hardware qualification,
...
def _make_module(
    ...
    * `decode_matmul_program` builds the two dense decode matmul configs. The
      ...
      (`recipes.dense_matmul_program_config`).
    """
