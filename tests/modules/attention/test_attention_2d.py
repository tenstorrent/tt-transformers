...
from tt_transformers.modules.attention import attention_2d
...
from tt_transformers.modules.rmsnorm.rmsnorm_2d import RMSNorm2DGeometry
...
class _Arch(Enum):
    WORMHOLE_B0 = 1
    # Both Galaxy architectures, plus one that has no Galaxy. `_architecture_name`
    # reads `.name`, so these stand in for `ttnn.device.Arch` members exactly as
...
@pytest.mark.parametrize(
    ("arch", "family"),
    [
        (_Arch.WORMHOLE_B0, "wormhole"),
        (_Arch.BLACKHOLE, "blackhole"),
    ],
    ids=str,
)
def test_both_galaxy_architectures_resolve_and_normalize_to_their_family(arch, family):
    """The gate `e48c329` missed, and the field it writes.

    `Attention2D` carried its own Wormhole-only architecture check while the
    other six 2D modules were widened onto `modules/galaxy_mesh.py`, so a
    Blackhole Galaxy was refused by name -- and separately,
    `architecture="wormhole"` was hardcoded into the resolved config, which is
    what the QK-norm sub-modules' own architecture field reads. Both halves are
    asserted here, because the second would have stayed invisible behind the
    first.

    The stored value is the *family*, not the spelling: the field has one
    canonical value while the gate accepts `Arch.WORMHOLE_B0`, `"wormhole_b0"`
    and `"wormhole"` alike.
    """

    config = resolve_attention2d_config(_config(architecture=arch, mesh=_Mesh(arch=arch)))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Retargeted from `arch="blackhole"`, which this module now ACCEPTS:
        # Blackhole Galaxy is a 32-device (8, 4) mesh declaring the same
        # `device_topology { dims: [8, 4] }`, so refusing it by name refused a
        # mesh Attention2D can describe. `e48c329` made the same move for the
        # other six 2D modules and retargeted their fail-closed rows the same
        # way. `QUASAR` is the negative control throughout this branch: a real
        # member of the pinned ttnn's `Arch` enum -- naming one that does not
        # exist raises AttributeError inside the gate and silently disables the
        # check -- that genuinely has no Galaxy.
        ({"mesh_device": _Mesh(arch=_Arch.QUASAR)}, "requires a Galaxy architecture"),
...
def test_wo_weight_memory_config_defaults_to_the_shared_placement():
    """Left unset, `wo` takes `weight_memory_config` — the default before `wo` got its own field."""
...
def test_activation_placement_and_dtype_fail_before_compute(host_ttnn, mode, tensor, message):
    ...
    if mode == "decode":
        ...
        def call():
            return model.decode_forward(tensor, "rot", DecodeMetadata(_Tensor("positions", shape=(32,)), _page_table()))

    else:

        def call():
            return model.prefill_forward(tensor, "rot", PrefillMetadata(128, page_table=_page_table()))
...
def test_module_owns_direct_ttnn_stages_and_has_no_legacy_or_model_imports():
    ...
    assert "tt_transformers.models" not in source
