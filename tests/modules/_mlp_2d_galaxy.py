...
lives in a shared module under `tests/modules/`.
"""
...
import ttnn
from tests.modules._wh_galaxy_hardware import galaxy_mode_plan, galaxy_prefetch_decode_mode_plan
    ...
    def plans(sequence: int, *, decode: bool = False) -> tuple[GalaxyCollectivePlan, ...]:
        ...
        return axis1_collectives + (
            collective_plan(
                ...
                # `50 * 1024` is **Wormhole's 50 worker cores** times the
                # `(TILE, 1024)` shard `decode_all_reduce_buffer_memcfg` uses,
                # not a free constant: a width-sharded tensor's width must equal
                # cores x shard width. Correct here, because this helper is
                # Wormhole-only. **Do not reuse it for Blackhole**, which has 100
                # worker cores and therefore needs 100 * 1024 — the production
                # path derives it in `plans._width_sharded_tensor_width` for
                # exactly this reason.
