...
`tests.modules._mlp_2d_galaxy`, so the PCC numbers here are directly
...
from tests.modules._mlp_2d_galaxy import (
    assert_mlp_pcc,
...
def _assert_activation(recorder: _SubdeviceRecorder, prefetcher: Prefetcher2D, payload: _Payload, mode: str) -> None:
    ...
    assert loads[-1].payload is prefetcher.context(mode).sub_device_manager_id, (
        f"activating {mode} loaded the wrong subdevice manager"
    )
    assert stalls, f"activating {mode} set no stall group"
    assert stalls[-1].payload == payload.plan(mode).stall_group, (
...
def test_prefetcher_2d_wh_galaxy_cleanup_from_active_mode_frees_the_mesh(mesh_device, active_mode, expect_error):
    ...
    try:
        ...
        # `create_global_circular_buffer` fails on L1. This was isolated in a
        # dedicated probe, where
...
def test_prefetcher_2d_wh_galaxy_attention_decode_with_active_prefetch(mesh_device):
    ...
    """
    from tests.modules.attention import test_attention_2d_wh_galaxy as attention_suite
