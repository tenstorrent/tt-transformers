...
def _context(mesh, mode):
    ...
    return SimpleNamespace(
        ...
        # Part of the context contract, and the non-persistent reduce-scatter
        # branch asks for it unconditionally. `None` is what the real context
        # returns for a partition starting at the grid origin -- every Wormhole
        # partition this op is legal under.
        reduce_scatter_workers_per_link=lambda num_links: None,
    )
...
def test_decode_uses_configured_activation_and_releases_only_module_owned_transients(monkeypatch, activation):
    ...
    module.config = SimpleNamespace(
        ...
        # The fused decode path, which is what this test's monkeypatched
        # `llama_rs_matmul` / `llama_reduce_scatter` stand-ins describe. Stated
        # rather than defaulted: `decode_forward` reads the field directly, so a
        # stand-in config that omits it is under-specified, and leaving the
        # module to `getattr(..., True)` would let a real typo resolve to the
        # fused path silently on Blackhole -- where those two ops move no data
        # and raise nothing.
        decode_fused_ccl=True,
    )
...
@pytest.mark.host
def test_unfused_decode_replaces_only_stages_one_and_two(monkeypatch):
    """`decode_fused_ccl=False` is the whole of the Blackhole decode delta.

    `llama_rs_matmul` and `llama_reduce_scatter` are two of the five fused
    galaxy CCLs that use 1D-multicast writers and therefore **no-op** on a
    2D-torus fabric -- they move no data and raise nothing, so the collective
    appears to run. `decode_forward` called both unconditionally, which is why
    Blackhole decode was not expressible at all.

    Stages 3-7 were already non-fused, so this asserts the narrow claim: the
    two fused ops are not reached, two `ttnn.linear` calls and two
    `_reduce_scatter_axis1` calls take their place carrying the `"w1"` and
    `"w3"` sequence keys, and everything downstream is untouched.

    The keys are the load-bearing part. Both reductions' outputs are live
    simultaneously in stage 3's gated multiply, so without distinct keys they
    alias one persistent output buffer and the multiply reads W3 twice with
    nothing raised. `_resolve_mlp2d_config` refuses the no-selector case for
    exactly that reason; this asserts the keys actually differ.
    """
    ...
    module.w1, module.w2, module.w3 = "w1", "w2", "w3"
    module.load_device_weights = lambda *_args: None
    # The two fused ops must not be reached at all.
    module._double_matmul_reduce_scatter_axis1 = MagicMock(side_effect=AssertionError("fused rs_matmul reached"))
    module._llama_reduce_scatter_axis1 = MagicMock(side_effect=AssertionError("fused reduce_scatter reached"))
    module._reduce_scatter_axis1 = MagicMock(side_effect=[w1_scattered, w3_scattered])
    module._all_gather_axis1 = MagicMock(return_value=gathered)
    module._all_reduce_tg = MagicMock(return_value=final)
    monkeypatch.setattr(mlp_2d, "_load_input_device_tensor", lambda value, *_args, **_kwargs: value)
    monkeypatch.setattr(ttnn, "linear", MagicMock(side_effect=[w1_projection, w3_projection, w2_output]))
    monkeypatch.setattr(ttnn, "mul", MagicMock(return_value=gated))
    monkeypatch.setattr(ttnn, "reshape", lambda value, _shape: value)
    deallocate = MagicMock()
    monkeypatch.setattr(ttnn, "deallocate", deallocate)
    ...
    module._double_matmul_reduce_scatter_axis1.assert_not_called()
    module._llama_reduce_scatter_axis1.assert_not_called()

    # Two projections through the decode configs, then W2 on the gathered input.
    assert [call.args[1] for call in ttnn.linear.call_args_list] == ["w1", "w3", "w2"]
    for call in ttnn.linear.call_args_list[:2]:
        assert call.args[0] is caller_input
        assert call.kwargs["program_config"] == "ff1-program"
        assert call.kwargs["memory_config"] == "ff1-memory"
        assert call.kwargs["dtype"] == "activation-dtype"

    # Distinct sequence keys, which is what stops the two reductions aliasing
    # one persistent buffer while both outputs are live.
    keys = [call.args[3] for call in module._reduce_scatter_axis1.call_args_list]
    assert keys == ["w1", "w3"]
    assert len(set(keys)) == 2
    for call in module._reduce_scatter_axis1.call_args_list:
        assert call.args[1] == "rs-memory"
        assert call.args[2] == "decode"

    # Stages 3-7 unchanged: the projections are the module's own transients and
    # are released; nothing the caller or a collective owns is.
    released = [call.args[0] for call in deallocate.call_args_list]
    assert w1_projection in released
    assert w3_projection in released
    assert caller_input not in released
    assert w1_scattered not in released
    assert w3_scattered not in released
    assert gathered not in released
    assert ttnn.mul.call_args.args[0] is w1_scattered
    assert ttnn.mul.call_args.args[1] is w3_scattered
