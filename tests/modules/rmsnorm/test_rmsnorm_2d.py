...
def test_head_local_decode_runs_the_kernel_in_the_compute_placement(monkeypatch):
    ...
    Measured on WH Galaxy `(8, 4)`. Both obvious spellings abort:
    ...
    * the created heads' own placement -> ``Height sharded inputs are not
    ...
    monkeypatch.setattr(
        rmsnorm_2d.ttnn,
        "to_memory_config",
        lambda *args, **kwargs: pytest.fail("to_memory_config is not partition-safe"),
    )
    norm_calls = []
    monkeypatch.setattr(rmsnorm_2d.ttnn, "rms_norm", lambda x, **kwargs: norm_calls.append((x, kwargs)) or normalized)
