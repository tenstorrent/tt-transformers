...
def test_decode_deallocates_all_owned_transients_and_preserves_borrowed_tensors(monkeypatch):
    ...
    # (D-C22). A bare `MagicMock` answers "yes" to everything, so say no
...
def test_decode_flattens_a_sharded_input_before_it_typecasts(monkeypatch):
    """D-C22: the sharded LM head output is made interleaved first.
    ...
    two cores that belong to no sub-device. The device evidence is
    `tttv2_milestone_c_evidence/trace/logs/p4_ring_sharded_prefix.log` (this
    placement, aborting) against `p5_ring_sharded_fixed.log` (the same test with
    this flattening in place).
    """
...
def test_replicated_users_place_every_per_user_buffer_on_every_column():
    """D-C23: `replicate_users` matches the placement the decode graph produces.
