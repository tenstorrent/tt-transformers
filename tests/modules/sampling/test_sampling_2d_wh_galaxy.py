...
def test_sampling_2d_wh_galaxy_subdevice_ordering_on_program_cache_hit(mesh_device):
    """Repeated decode sampling stays correct under the loaded decode partition (D-C12).
...
def test_sampling_2d_wh_galaxy_bfloat8_b_logits_under_the_decode_partition(mesh_device):
    """Sampled decode on the LM head's own dtype, inside the loaded decode partition (D-C22).
    ...
    (`tttv2_milestone_c_evidence/trace/logs/x1_L_sampling_probe.log`, the first
    run in this project to get past the D-C21 hang that sat upstream of it.)
...
def test_sampling_2d_wh_galaxy_ring_sharded_logits_under_the_decode_partition(mesh_device):
    """Sampled decode on the LM head's own *placement*, inside the decode partition (D-C22).
    ...
    interleaved in DRAM. That single difference is the whole of D-C22.
    ...
    *interleaved* input. Given a sharded one it resolves the program's cores
    ...
    Measured three ways rather than argued. `logs/x1_L_sampling_probe.log` and
    `logs/X5_L_sampled.log` are the executor aborting at `sampling_2d.py:445`;
    `logs/x6_typecast_placement.log` is the identical call on an *interleaved*
    tensor of the same shape and dtype under the identical partition, which
    **places**; and `logs/p1_L_sampling_context.log` prints the real tensor's
    memory config at the call site and then tries seven placements of it in
    situ - only the two that go through an interleaved tensor first survive.
    """
...
def test_sampling_2d_wh_galaxy_replicated_users_on_the_lm_head_placement(mesh_device):
    """The placement the Galaxy decode graph actually produces (D-C23).
    ...
    *vocabulary* over the eight rows, so `lm_head_2d` hands the sampler all 32
    ...
    (`tttv2_milestone_c_evidence/trace/logs/p6_L_sampling_probe_fixed.log`.)
