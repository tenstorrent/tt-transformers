...
def _sampler(mesh_device):
    """Build Sampling2D on the geometry qualified by the greedy sampling test."""
...
def test_sampling_2d_wh_galaxy_stochastic_token_in_valid_set(
    ...
    Calibrated on this host across three fresh processes: the observed violation count
    was 0 in every case, in every invocation, including both ``p in (0, 1)`` nucleus
    cases. The bound of 1 on those two is one above that observed maximum - deliberate
    headroom for the bfloat16 softmax/cumsum boundary noted in the parametrization, which
    did not manifest on this geometry at all once the reciprocal-temperature defect in
    ``_update_call_buffers`` was fixed. Pre-fix, ``k32-p0.5-t0.8`` showed 4/32. A run
    that reports violations here is a regression, not noise.
    """
...
def test_sampling_2d_wh_galaxy_stochastic_excludes_padded_vocab(mesh_device):
    ...
    The greedy sampling test cannot make this assertion: with ``k=1`` a single
    ...
    try:
        for invocation in range(8):
            ...
            try:
                tokens = _sampled_tokens(output)
                assert torch.all(tokens < VOCAB_SIZE), (
...
def test_sampling_2d_wh_galaxy_seeded_sampling_is_repeatable_and_slot_stable(mesh_device):
    ...
    try:
        ...
        assert not torch.equal(perturbed[16:], first[16:]), (
...
def test_sampling_2d_wh_galaxy_per_slot_heterogeneous_parameters(mesh_device):
    ...
    * slots 24-31 - ``k=32, p=0.9, temp=0.8``: nucleus containment, calibrated tolerance.
    ...
    Calibrated on this host across three fresh processes: the slots 24-31 violation count
    was 0 in every invocation. Its bound of 1 is one above that observed maximum -
    deliberate headroom for the bfloat16 softmax/cumsum nucleus boundary, which did not
    manifest here once the reciprocal-temperature defect in ``_update_call_buffers`` was
    fixed. Pre-fix, this group showed 2/8. A run that reports violations here is a
    regression, not noise. The other three groups admit no violation at all.
    """
